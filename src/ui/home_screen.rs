...
pub(crate) fn load_logo_texture(
    ctx: &egui::Context,
    img: image::DynamicImage,
) -> Option<egui::TextureHandle> {
    ...
    for pixel in pixels.chunks_exact_mut(4) {
        if pixel[0].max(pixel[1]).max(pixel[2]) < 18 {
            pixel[3] = 0;
        }
    }
...
fn thumb_for(
    ...
    let img = crate::ui::embedded_assets::open_image(path).ok()?;
...
fn home_nav(ctx: &egui::Context) -> HomeNav {
    ctx.data_mut(|d| *d.get_temp_mut_or_insert_with(egui::Id::new("home_nav"), || HomeNav::Home))
}
...
fn interact_target_nav(
    ...
    if ui
        .interact(rect, egui::Id::new((id_scope, label)), egui::Sense::click())
        .clicked()
    {
...
fn summarize_project(path: &std::path::Path) -> Option<(usize, usize)> {
    read_project_json(path)
        .and_then(|v| summarize_value(&v).map(|s| (s.comp_names.len(), s.layers)))
}
...
pub(crate) fn new_project_from_header(app: &mut KagariApp) {
...
pub(crate) fn open_project_dialog(app: &mut KagariApp) {
...
pub(crate) fn import_footage_dialog(app: &mut KagariApp) {
    if let Some(path) = rfd::FileDialog::new()
        .add_filter(
            "Footage",
            &[
                "png", "jpg", "jpeg", "webp", "bmp", "exr", "mov", "mp4", "avi", "mkv", "webm",
                "av1", "wav", "mp3",
            ],
        )
        .pick_file()
...
fn open_selected_in_studio(app: &mut KagariApp, path: &std::path::Path) {
    ...
    match ext.as_str() {
        ...
        _ => {
            app.toasts
                .info("Entered studio — use File > Import for unsupported media");
        }
    }
}
...
fn import_video_into_active_comp(app: &mut KagariApp, path: &std::path::Path) {
    ...
    app.toasts.info(format!(
        "Extracting video frames for '{}' via FFmpeg...",
        name
    ));
...
pub fn draw(app: &mut KagariApp, ctx: &egui::Context) {
    ...
    let should_reset_search =
        ctx.data_mut(|d| d.get_temp::<bool>(search_reset_id).unwrap_or(false));
    ...
    let landing_mode = matches!(home_nav(ctx), HomeNav::Home) && ctx.screen_rect().width() >= 900.0;
        ...
        .show(ctx, |ui| {
            ...
            match home_nav(ctx) {
                ...
                HomeNav::Projects | HomeNav::Recent | HomeNav::Starred => {
                    draw_reference_project_browser_page(app, ui, ctx)
                }
...
fn draw_settings_target(app: &mut KagariApp, ctx: &egui::Context) {
    egui::TopBottomPanel::top("settings_target_window_bar")
        ...
        .show(ctx, |ui| {
            ...
            ui.painter().line_segment(
                [
                    egui::pos2(0.0, 42.0),
                    egui::pos2(ui.max_rect().right(), 42.0),
                ],
    ...
    egui::CentralPanel::default()
        ...
        .show(ctx, |ui| {
            ...
            let category = egui::Rect::from_min_max(
                egui::pos2(left.right(), r.top()),
                egui::pos2(left.right() + 236.0, r.bottom()),
            );
            ...
            ui.painter().line_segment(
                [
                    egui::pos2(category.right(), r.top()),
                    egui::pos2(category.right(), r.bottom()),
                ],
                egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE),
            );
            ...
            if let Some(texture) = app.home_banner.as_ref() {
                ui.allocate_new_ui(
                    egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
                        egui::pos2(36.0, r.top() + 19.0),
                        egui::vec2(78.0, 78.0),
                    )),
                    |icon_ui| {
                        icon_ui.add(egui::Image::new(egui::load::SizedTexture::new(
                            texture.id(),
                            egui::vec2(78.0, 78.0),
                        )));
                    },
            ...
            let nav = [
                ...
                (crate::ui::icons::SVG_FOLDER, "プロジェクトを開く", 196.0),
                (crate::ui::icons::SVG_FILE_PLUS, "新規プロジェクト", 252.0),
                (crate::ui::icons::SVG_BOOK, "チュートリアル", 342.0),
                (crate::ui::icons::SVG_DOCUMENT, "ドキュメント", 398.0),
                (crate::ui::icons::SVG_SETTINGS, "設定", 498.0),
                (crate::ui::icons::SVG_POWER, "終了", 554.0),
            ];
            for (icon, label, y) in nav {
                let active = label == "設定";
                let nav_rect = egui::Rect::from_min_max(
                    egui::pos2(17.0, r.top() + y - 5.0),
                    egui::pos2(left.right(), r.top() + y + 47.0),
                );
                if active {
                    ui.painter()
                        .rect_filled(nav_rect, 0.0, crate::ui::theme::colors::BG_PANEL_DARK);
                ...
                crate::ui::icons::render_svg_at(
                    ui,
                    format!("settings-target-nav-{label}"),
                    icon,
                    egui::vec2(28.0, 28.0),
                    if active {
                    ...
                    egui::pos2(40.0, r.top() + y + 5.0),
                );
                ui.painter().text(
                    egui::pos2(87.0, r.top() + y + 19.0),
                    egui::Align2::LEFT_CENTER,
                    label,
            ...
            ui.painter().text(
                egui::pos2(category.left() + 30.0, r.top() + 44.0),
                egui::Align2::LEFT_CENTER,
                "設定",
            ...
            let categories = [
                (crate::ui::icons::SVG_SETTINGS, "一般"),
                (crate::ui::icons::SVG_PALETTE, "外観"),
                (crate::ui::icons::SVG_GPU, "パフォーマンス"),
                (crate::ui::icons::SVG_KEYBOARD, "ショートカット"),
                (crate::ui::icons::SVG_FOLDER, "メディア"),
                (crate::ui::icons::SVG_AUDIO, "オーディオ"),
                (crate::ui::icons::SVG_LAYERS, "プラグイン"),
                (crate::ui::icons::SVG_FILE, "保存"),
            ];
            ...
            for (index, (icon, label)) in categories.into_iter().enumerate() {
                ...
                if active {
                    ui.painter()
                        .rect_filled(row_rect, 0.0, crate::ui::theme::colors::BG_PANEL_DARK);
                    ui.painter().rect_filled(
                        egui::Rect::from_min_size(
                            egui::pos2(category.left() + 17.0, y),
                            egui::vec2(4.0, 53.0),
                ...
                crate::ui::icons::render_svg_at(
                    ui,
                    format!("settings-target-category-{index}"),
                    icon,
                    egui::vec2(25.0, 25.0),
                    if active {
                        crate::ui::theme::colors::ACCENT_BRAND
                    ...
                    egui::pos2(category.left() + 37.0, y + 14.0),
                );
                ui.painter().text(
                    egui::pos2(category.left() + 82.0, y + 27.0),
                    egui::Align2::LEFT_CENTER,
                    label,
                ...
                let response = ui.interact(
                    row_rect,
                    egui::Id::new(("settings-target-category-row", index)),
                    egui::Sense::click(),
                );
                if response.clicked() {
...
fn draw_settings_target_responsive(app: &mut KagariApp, ctx: &egui::Context) {
    ...
    egui::CentralPanel::default()
        ...
        .show(ctx, |ui| {
            ...
            let category = egui::Rect::from_min_max(
                egui::pos2(left.right(), r.top()),
                egui::pos2(left.right() + category_width, r.bottom()),
            );
            let content = egui::Rect::from_min_max(category.right_top(), r.right_bottom());
            ui.painter()
                .rect_filled(left, 0.0, crate::ui::theme::colors::BG_PANEL_BASE);
            ...
            if let Some(texture) = app.home_banner.as_ref() {
                let x = if narrow {
                    (left_width - logo_size) * 0.5
                } else {
                    28.0
                };
                ui.allocate_new_ui(
                    egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
                        egui::pos2(x, r.top() + 18.0),
            ...
            let nav = [
                (crate::ui::icons::SVG_HOME, "ホーム"),
                (crate::ui::icons::SVG_FOLDER, "プロジェクトを開く"),
                (crate::ui::icons::SVG_FILE_PLUS, "新規プロジェクト"),
                (crate::ui::icons::SVG_BOOK, "チュートリアル"),
                (crate::ui::icons::SVG_DOCUMENT, "ドキュメント"),
                (crate::ui::icons::SVG_SETTINGS, "設定"),
                (crate::ui::icons::SVG_POWER, "終了"),
            ];
            for (index, (icon, label)) in nav.into_iter().enumerate() {
                ...
                let nav_rect = egui::Rect::from_min_max(
                    egui::pos2(0.0, y - 6.0),
                    egui::pos2(left.right(), y + 43.0),
                );
                if active {
                    ui.painter()
                        .rect_filled(nav_rect, 0.0, crate::ui::theme::colors::BG_PANEL_DARK);
                    ui.painter().rect_filled(
                        egui::Rect::from_min_size(egui::pos2(0.0, y - 6.0), egui::vec2(4.0, 49.0)),
                        0.0,
                        crate::ui::theme::colors::ACCENT_BRAND,
                    );
                }
                let icon_x = if narrow {
                    (left_width - 24.0) * 0.5
                } else {
                    28.0
                };
                crate::ui::icons::render_svg_at(
                    ui,
                    format!("settings-responsive-nav-{index}"),
                    icon,
                    egui::vec2(24.0, 24.0),
                    if active {
                        crate::ui::theme::colors::ACCENT_BRAND_HOVER
                    ...
                    egui::pos2(icon_x, y + 5.0),
                );
                if !narrow {
                    ui.painter().text(
                        egui::pos2(72.0, y + 17.0),
                        egui::Align2::LEFT_CENTER,
                        label,
                        egui::FontId::proportional(14.0),
                        if active {
            ...
            if !narrow {
                ui.painter().text(
                    egui::pos2(category.left() + 20.0, r.top() + 36.0),
                    egui::Align2::LEFT_CENTER,
                    "設定",
            ...
            for (index, (icon, label)) in categories.into_iter().enumerate() {
                let y = r.top()
                    + if narrow { 35.0 } else { 76.0 }
                    + index as f32 * if narrow { 44.0 } else { 49.0 };
                let active = index == selected_category;
                let row_rect = egui::Rect::from_min_max(
                    egui::pos2(category.left(), y),
                    egui::pos2(category.right(), y + 46.0),
                );
                if active {
                    ui.painter()
                        .rect_filled(row_rect, 0.0, crate::ui::theme::colors::BG_PANEL_DARK);
                ...
                crate::ui::icons::render_svg_at(
                    ui,
                    format!("settings-responsive-category-{index}"),
                    icon,
                    egui::vec2(21.0, 21.0),
                    if active {
                        crate::ui::theme::colors::ACCENT_BRAND
                    ...
                    egui::pos2(
                        if narrow {
                            category.left() + (category_width - 21.0) * 0.5
                        } else {
                            category.left() + 20.0
                        },
                        y + 12.0,
                ...
                if !narrow {
                    ui.painter().text(
                        egui::pos2(category.left() + 53.0, y + 23.0),
                        egui::Align2::LEFT_CENTER,
                        label,
                ...
                if ui
                    .interact(
                        row_rect,
                        egui::Id::new(("settings-responsive-category-row", index)),
                        egui::Sense::click(),
                    )
                    .clicked()
                {
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                for (title, rows) in cards {
                    egui::Frame::none()
                        .fill(crate::ui::theme::colors::BG_PANEL_BASE)
                        .stroke(egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE))
                        .rounding(7.0)
...
fn draw_export_target(app: &mut KagariApp, ctx: &egui::Context) {
    ...
    let left_width = if width >= 1200.0 {
        264.0
    } else if narrow {
        68.0
    } else {
    ...
    egui::CentralPanel::default()
        ...
        .show(ctx, |ui| {
            ...
            if !narrow {
                ...
                ui.painter().text(
                    egui::pos2(if width >= 1200.0 { 190.0 } else { 167.0 }, r.top() + 59.0),
                    egui::Align2::LEFT_CENTER,
                    "VFX",
                    egui::FontId::proportional(if width >= 1200.0 { 27.0 } else { 20.0 }),
                    crate::ui::theme::colors::TEXT_SECONDARY,
            ...
            let nav = [
                ...
                (crate::ui::icons::SVG_FOLDER, "プロジェクトを開く"),
                (crate::ui::icons::SVG_FILE_PLUS, "新規プロジェクト"),
                (crate::ui::icons::SVG_LAYERS, "編集"),
                (crate::ui::icons::SVG_EFFECTS, "エフェクト"),
                (crate::ui::icons::SVG_FOLDER, "素材"),
                (crate::ui::icons::SVG_AUDIO, "オーディオ"),
                (crate::ui::icons::SVG_TOOL_TEXT, "テキスト"),
                (crate::ui::icons::SVG_ARROW_RIGHT, "トランジション"),
                (crate::ui::icons::SVG_PALETTE, "カラー"),
                (crate::ui::icons::SVG_EXPORT, "書き出し"),
                (crate::ui::icons::SVG_BOOK, "チュートリアル"),
                (crate::ui::icons::SVG_DOCUMENT, "ドキュメント"),
                (crate::ui::icons::SVG_SETTINGS, "設定"),
                (crate::ui::icons::SVG_POWER, "終了"),
            ...
            for (index, (icon, label)) in nav.into_iter().enumerate() {
                let y = start + index as f32 * if width >= 1200.0 { 41.0 } else { 36.0 };
                let active = label == "書き出し";
                let nav_rect = egui::Rect::from_min_max(
                    egui::pos2(10.0, y - 5.0),
                    egui::pos2(sidebar.right(), y + 37.0),
                );
                if active {
                    ui.painter()
                        .rect_filled(nav_rect, 0.0, crate::ui::theme::colors::BG_PANEL_DARK);
                    ui.painter().rect_filled(
                        egui::Rect::from_min_size(egui::pos2(10.0, y - 5.0), egui::vec2(4.0, 42.0)),
                        0.0,
                        crate::ui::theme::colors::ACCENT_BRAND,
                    );
                }
                ...
                if !narrow {
                    ui.painter().text(
                        egui::pos2(80.0, y + 16.0),
                        egui::Align2::LEFT_CENTER,
                        label,
                        egui::FontId::proportional(if width >= 1200.0 { 14.0 } else { 11.0 }),
                        if active {
                            colors::TEXT_PRIMARY
                        } else {
                            crate::ui::theme::colors::TEXT_SECONDARY_BRIGHT
                ...
                interact_target_nav(ui, ctx, nav_rect, label, "export-target-nav");
            }
            if !narrow && !short {
                ui.painter().text(
                    egui::pos2(32.0, r.bottom() - 27.0),
                    egui::Align2::LEFT_CENTER,
                    "Kagari VFX   v0.1.0",
                    egui::FontId::proportional(13.0),
                    colors::TEXT_SECONDARY,
            ...
            let main =
                egui::Rect::from_min_max(egui::pos2(sidebar.right(), r.top()), r.right_bottom());
            let margin = if width >= 1200.0 { 34.0 } else { 16.0 };
            let title_x = main.left() + margin;
            ui.painter().text(
                egui::pos2(title_x, r.top() + 43.0),
                egui::Align2::LEFT_CENTER,
                "書き出し設定",
                egui::FontId::proportional(if width >= 1200.0 { 32.0 } else { 23.0 }),
                colors::TEXT_PRIMARY,
            );
            ui.painter().text(
                egui::pos2(title_x, r.top() + 79.0),
                egui::Align2::LEFT_CENTER,
                "あなたの作品を、世界へ。高品質な映像を書き出しましょう。",
                egui::FontId::proportional(if width >= 1200.0 { 17.0 } else { 12.0 }),
                colors::TEXT_SECONDARY,
            );
            let preview_w = if width >= 1200.0 {
                703.0
            } else {
                ((main.width() - margin * 2.0 - 14.0) * 0.5).max(280.0)
            };
            let settings_w = if width >= 1200.0 {
                main.width() - margin * 2.0 - preview_w - 14.0
            } else {
                preview_w
            };
            let top = r.top() + 118.0;
            let preview = egui::Rect::from_min_size(
                egui::pos2(title_x, top),
                egui::vec2(preview_w, if width >= 1200.0 { 449.0 } else { 260.0 }),
            );
            let settings = egui::Rect::from_min_size(
                egui::pos2(
                    if width >= 1200.0 {
                        preview.right() + 14.0
                    } else {
                        title_x + preview_w + 14.0
                    },
                    top - 34.0,
                ),
                egui::vec2(settings_w, if width >= 1200.0 { 482.0 } else { 400.0 }),
            );
            let fill = crate::ui::theme::colors::BG_PANEL_BASE;
            let stroke = egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE);
            ui.painter().rect(preview, 8.0, fill, stroke);
            ui.painter().rect(settings, 8.0, fill, stroke);
            let image_rect = egui::Rect::from_min_size(
                egui::pos2(preview.left() + 18.0, preview.top() + 15.0),
                egui::vec2(
                    preview.width() - 36.0,
                    if width >= 1200.0 { 352.0 } else { 155.0 },
                ),
            );
            if let Some(id) = reference_texture(app, ctx, "render_preview.webp") {
                ui.painter().image(
                    id,
                    image_rect,
            ...
            ui.painter().rect_stroke(image_rect, 0.0, stroke);
            ui.painter().text(
                egui::pos2(preview.left() + 18.0, preview.bottom() - 55.0),
                egui::Align2::LEFT_CENTER,
                "00:00:04:12",
                egui::FontId::proportional(16.0),
                crate::ui::theme::colors::ACCENT_BRAND,
            ...
            ui.painter().line_segment(
                [
                    egui::pos2(preview.left() + 135.0, preview.bottom() - 56.0),
                    egui::pos2(preview.right() - 115.0, preview.bottom() - 56.0),
                ],
                egui::Stroke::new(3.0_f32, crate::ui::theme::colors::ACCENT_BRAND),
            );
            for (i, icon) in [
            ...
            .enumerate()
            {
                crate::ui::icons::render_svg_at(
                    ui,
                    format!("export-preview-transport-{i}"),
                    icon,
                    egui::vec2(16.0, 16.0),
                    colors::TEXT_PRIMARY,
                    egui::pos2(
                        preview.center().x - 83.0 + i as f32 * 44.0,
                        preview.bottom() - 35.0,
                    ),
                );
            }
            ui.painter().text(
                egui::pos2(settings.left() + 20.0, settings.top() + 28.0),
                egui::Align2::LEFT_CENTER,
                "ビデオ        オーディオ        詳細設定",
                egui::FontId::proportional(if width >= 1200.0 { 14.0 } else { 11.0 }),
                colors::TEXT_PRIMARY,
            ...
            ui.painter().line_segment(
                [
                    egui::pos2(settings.left() + 30.0, settings.top() + 42.0),
                    egui::pos2(settings.left() + 127.0, settings.top() + 42.0),
                ],
            ...
            let field_x = settings.left()
                + if width >= 1200.0 {
                    214.0
                } else {
                    settings.width() * 0.35
                };
            for (i, (lab, val)) in rows.into_iter().enumerate() {
                let yy = settings.top() + 72.0 + i as f32 * 38.0;
                ui.painter().text(
                    egui::pos2(settings.left() + 20.0, yy + 11.0),
                    egui::Align2::LEFT_CENTER,
                    lab,
                    egui::FontId::proportional(if width >= 1200.0 { 14.0 } else { 10.0 }),
                    colors::TEXT_SECONDARY,
                );
                ui.painter().rect(
                    egui::Rect::from_min_size(
                        egui::pos2(field_x, yy - 5.0),
                ...
                ui.painter().text(
                    egui::pos2(field_x + 12.0, yy + 11.0),
                    egui::Align2::LEFT_CENTER,
                    val,
                    egui::FontId::proportional(if width >= 1200.0 { 13.0 } else { 9.0 }),
                    colors::TEXT_PRIMARY,
                );
            }
            let checkbox = |y: f32, label: &str, checked: bool| {
                let box_rect = egui::Rect::from_min_size(
                    egui::pos2(settings.left() + 214.0, y - 9.0),
                    egui::vec2(18.0, 18.0),
                );
                ui.painter().rect(
                    box_rect,
                ...
                ui.painter().text(
                    egui::pos2(settings.left() + 244.0, y),
                    egui::Align2::LEFT_CENTER,
                    label,
                    egui::FontId::proportional(if width >= 1200.0 { 13.0 } else { 9.0 }),
                    colors::TEXT_SECONDARY,
            ...
            checkbox(settings.top() + 295.0, "HDR（Rec. 2100 / HLG）", false);
            ui.painter().line_segment(
                [
                    egui::pos2(settings.left() + 20.0, settings.top() + 319.0),
                    egui::pos2(settings.right() - 20.0, settings.top() + 319.0),
                ],
                stroke,
            );
            for (i, (lab, val)) in [
                ("保存先", "~/Movies/Kagari VFX"),
                ("ファイル名", "Sample Project"),
            ]
            .into_iter()
            .enumerate()
            {
                let yy = settings.top() + 338.0 + i as f32 * 38.0;
                ui.painter().text(
                    egui::pos2(settings.left() + 20.0, yy + 11.0),
                    egui::Align2::LEFT_CENTER,
                    lab,
            ...
            let export_action = egui::Rect::from_min_size(
                egui::pos2(settings.left() + 20.0, settings.bottom() - 60.0),
                egui::vec2(settings.width() - 40.0, 47.0),
            );
            ui.painter().rect(
                export_action,
                6.0,
            ...
            crate::ui::icons::render_svg_at(
                ui,
                "export-start-icon".to_string(),
                crate::ui::icons::SVG_EXPORT,
                egui::vec2(18.0, 18.0),
                egui::Color32::WHITE,
                egui::pos2(settings.center().x - 84.0, settings.bottom() - 45.0),
            );
            ui.painter().text(
                egui::pos2(settings.center().x + 10.0, settings.bottom() - 36.0),
                egui::Align2::CENTER_CENTER,
                "書き出し開始",
                egui::FontId::proportional(if width >= 1200.0 { 15.0 } else { 11.0 }),
                egui::Color32::WHITE,
            );
            if ui
                .interact(
                    export_action,
                    egui::Id::new("reference-export-start"),
                    egui::Sense::click(),
                )
                .clicked()
            {
            ...
            let queue_y = if width >= 1200.0 {
                r.top() + 582.0
            } else {
                settings.bottom() + 14.0
            };
            let queue = egui::Rect::from_min_size(
                egui::pos2(title_x, queue_y),
                egui::vec2(
                    main.width() - margin * 2.0,
                    (height - queue_y - r.top() - 34.0).max(150.0),
                ),
            );
            let p = ui.painter();
            let queue_title_y = if short { 22.0 } else { 31.0 };
            let queue_header_y = if short { 45.0 } else { 75.0 };
            let queue_row_start = if short { 70.0 } else { 113.0 };
            let queue_row_step = if short { 29.0 } else { 47.0 };
            p.rect(queue, 8.0, fill, stroke);
            p.text(
                egui::pos2(queue.left() + 20.0, queue.top() + queue_title_y),
                egui::Align2::LEFT_CENTER,
                "レンダーキュー（3）",
                egui::FontId::proportional(if width >= 1200.0 { 17.0 } else { 13.0 }),
                colors::TEXT_PRIMARY,
            );
            if width >= 1200.0 {
                let headers = [
                ...
                for (offset, label) in headers {
                    p.text(
                        egui::pos2(queue.left() + offset, queue.top() + queue_header_y),
                        egui::Align2::LEFT_CENTER,
                        label,
                        egui::FontId::proportional(11.0),
                        colors::TEXT_MUTED,
                ...
                let rows = [
                    ...
                    (
                        ...
                        1.0_f32,
                        "100 %",
                        "recent_project_atlas.webp",
                    ),
                ];
                for (i, (name, format, resolution, fps, status, progress, percent, asset)) in
                    rows.into_iter().enumerate()
                {
                    let yy = queue.top() + queue_row_start + i as f32 * queue_row_step;
                    p.line_segment(
                        [
                            egui::pos2(queue.left() + 18.0, yy + 22.0),
                            egui::pos2(queue.right() - 18.0, yy + 22.0),
                        ],
                        egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE),
                    );
                    p.text(
                        egui::pos2(queue.left() + 20.0, yy),
                        egui::Align2::LEFT_CENTER,
                        format!("{}", i + 1),
                        egui::FontId::proportional(11.0),
                        colors::TEXT_SECONDARY,
                    ...
                    if let Some(id) = reference_texture(app, ctx, asset) {
                        let thumb = egui::Rect::from_min_size(
                            egui::pos2(queue.left() + 44.0, yy - 16.0),
                            egui::vec2(66.0, 36.0),
                        );
                        p.image(
                            id,
                    ...
                    let progress_rect = egui::Rect::from_min_size(
                        egui::pos2(queue.left() + 810.0, yy - 4.0),
                        egui::vec2(140.0, 8.0),
                    );
                    p.rect_filled(progress_rect, 4.0, colors::BORDER_MEDIUM);
                    p.rect_filled(
                        egui::Rect::from_min_size(
                            progress_rect.min,
                    ...
                    p.text(
                        egui::pos2(queue.left() + 1035.0, yy),
                        egui::Align2::LEFT_CENTER,
                        if i == 2 {
                            "サウンドで通知"
                        } else {
                ...
                for (i, c) in cols.into_iter().enumerate() {
                    p.text(
                        egui::pos2(
                            queue.left() + 20.0 + i as f32 * (queue.width() / 8.0),
                            queue.top() + queue_header_y,
                        ),
                        egui::Align2::LEFT_CENTER,
                        c,
                ...
                .enumerate()
                {
                    ...
                    p.text(
                        egui::pos2(queue.left() + queue.width() * 0.10, yy),
                        egui::Align2::LEFT_CENTER,
                        name,
                        egui::FontId::proportional(11.0),
                        colors::TEXT_PRIMARY,
                    ...
                    p.text(
                        egui::pos2(queue.left() + queue.width() * 0.50, yy),
                        egui::Align2::LEFT_CENTER,
                        status,
                        egui::FontId::proportional(11.0),
                        if i == 0 {
...
fn draw_settings_responsive_content(ui: &mut egui::Ui, content: egui::Rect, narrow: bool) {
    ...
    p.text(
        egui::pos2(content.left() + 22.0, content.top() + 35.0),
        egui::Align2::LEFT_CENTER,
        "一般",
        egui::FontId::proportional(if narrow { 19.0 } else { 22.0 }),
        white,
    );
    p.text(
        egui::pos2(content.left() + 22.0, content.top() + 63.0),
        egui::Align2::LEFT_CENTER,
        "アプリケーションの基本設定",
        egui::FontId::proportional(12.0),
        muted,
    ...
    for (index, title) in titles.into_iter().enumerate() {
        ...
        p.text(
            egui::pos2(rect.left() + 16.0, rect.top() + 23.0),
            egui::Align2::LEFT_CENTER,
            title,
            egui::FontId::proportional(14.0),
            white,
        ...
        for row_index in 0..3 {
            ...
            p.text(
                egui::pos2(rect.left() + 16.0, yy + 10.0),
                egui::Align2::LEFT_CENTER,
                ["テーマ", "言語", "UIのスケール"][row_index],
                egui::FontId::proportional(11.0),
                muted,
            );
            ...
            p.text(
                egui::pos2(field_left + 8.0, yy + 11.0),
                egui::Align2::LEFT_CENTER,
                ["ダーク", "日本語", "100%（推奨）"][row_index],
                egui::FontId::proportional(10.0),
                white,
            );
        }
    }
    let system = egui::Rect::from_min_size(
        egui::pos2(content.left() + 16.0, content.bottom() - 48.0),
        egui::vec2(content.width() - 32.0, 36.0),
    );
    p.rect(
        system,
        6.0,
    ...
    p.text(
        system.left_center() + egui::vec2(12.0, 0.0),
        egui::Align2::LEFT_CENTER,
        if narrow {
            "Kagari VFX v0.1.0"
        } else {
            "システム情報   Kagari VFX v0.1.0   │   macOS 14.0   │   Metal 対応"
        },
        egui::FontId::proportional(10.0),
        muted,
    );
...
fn draw_settings_target_content(ui: &mut egui::Ui, content: egui::Rect) {
    let p = ui.painter();
    let border = crate::ui::theme::colors::BG_ELEVATED;
    let card_fill = crate::ui::theme::colors::BG_PANEL_BASE;
    let white = colors::TEXT_PRIMARY;
    let muted = colors::TEXT_SECONDARY;
    p.text(
        egui::pos2(content.left() + 30.0, content.top() + 47.0),
        egui::Align2::LEFT_CENTER,
        "一般",
    ...
    p.text(
        egui::pos2(content.left() + 30.0, content.top() + 79.0),
        egui::Align2::LEFT_CENTER,
        "アプリケーションの基本設定を管理します。",
        egui::FontId::proportional(15.0),
        muted,
    ...
    p.text(
        egui::pos2(content.right() - 265.0, content.top() + 47.0),
        egui::Align2::LEFT_CENTER,
        "Create. Composite. Illuminate.",
        egui::FontId::proportional(14.0),
        colors::TEXT_MUTED,
    ...
    let label = |p: &egui::Painter, x: f32, y: f32, value: &str| {
        p.text(
            egui::pos2(x, y),
            egui::Align2::LEFT_CENTER,
            value,
    ...
    label(p, cards[0].0 + 21.0, cards[0].1 + 68.0, "テーマ");
    field(
        p,
        cards[0].0 + 190.0,
        cards[0].1 + 51.0,
    ...
    label(p, cards[0].0 + 21.0, cards[0].1 + 108.0, "アクセントカラー");
    for (i, color) in [
        crate::ui::theme::colors::ACCENT_BRAND,
        egui::Color32::from_rgb(61, 156, 236),
        egui::Color32::from_rgb(145, 93, 239),
        egui::Color32::from_rgb(218, 64, 181),
        egui::Color32::from_rgb(242, 72, 69),
        egui::Color32::from_rgb(245, 177, 27),
        egui::Color32::from_rgb(32, 196, 73),
    ]
    .into_iter()
    .enumerate()
    {
        p.circle_filled(
            egui::pos2(cards[0].0 + 204.0 + i as f32 * 38.0, cards[0].1 + 109.0),
            10.0,
            color,
        );
    }
    label(p, cards[0].0 + 21.0, cards[0].1 + 154.0, "言語");
    field(
        p,
        cards[0].0 + 190.0,
        cards[0].1 + 137.0,
    ...
    label(p, cards[0].0 + 21.0, cards[0].1 + 198.0, "UIのスケール");
    field(
        p,
        cards[0].0 + 190.0,
        cards[0].1 + 181.0,
    ...
    label(p, cards[1].0 + 21.0, cards[1].1 + 68.0, "自動保存の間隔");
    field(
        p,
        cards[1].0 + 261.0,
        cards[1].1 + 51.0,
    ...
    label(p, cards[1].0 + 21.0, cards[1].1 + 240.0, "既定の保存先");
    field(
        p,
        cards[1].0 + 221.0,
        cards[1].1 + 223.0,
    ...
    let switch_pill = |p: &egui::Painter, x: f32, y: f32| {
        p.rect(
            egui::Rect::from_min_size(egui::pos2(x, y), egui::vec2(48.0, 27.0)),
            14.0,
            egui::Color32::from_rgb(255, 104, 25),
            egui::Stroke::NONE,
        );
        p.circle_filled(egui::pos2(x + 36.0, y + 13.5), 10.0, egui::Color32::WHITE);
    };
    ...
    label(p, cards[2].0 + 21.0, cards[2].1 + 108.0, "使用するGPU");
    field(
        p,
        cards[2].0 + 190.0,
        cards[2].1 + 91.0,
    ...
    label(p, cards[2].0 + 21.0, cards[2].1 + 152.0, "メモリの使用上限");
    let meter_right = cards[2].0 + cards[2].2 - 66.0;
    p.line_segment(
        [
            egui::pos2(cards[2].0 + 190.0, cards[2].1 + 140.0),
            egui::pos2(meter_right - 18.0, cards[2].1 + 140.0),
        ],
        egui::Stroke::new(5.0_f32, crate::ui::theme::colors::ACCENT_BRAND),
    );
    p.circle_filled(
        egui::pos2(
            cards[2].0 + 190.0 + (cards[2].2 - 274.0) * 0.65,
            cards[2].1 + 140.0,
        ),
        8.0,
        white,
    );
    label(p, meter_right, cards[2].1 + 140.0, "75%");
    label(
        p,
        cards[2].0 + 21.0,
        cards[2].1 + 194.0,
    ...
    label(p, cards[3].0 + 21.0, cards[3].1 + 108.0, "プロキシ解像度");
    field(
        p,
        cards[3].0 + 290.0,
        cards[3].1 + 91.0,
    ...
    label(p, cards[3].0 + 21.0, cards[3].1 + 240.0, "スクロール動作");
    field(
        p,
        cards[3].0 + 290.0,
        cards[3].1 + 223.0,
    ...
    label(p, cards[4].0 + 21.0, cards[4].1 + 68.0, "フォーマット");
    field(
        p,
        cards[4].0 + 168.0,
        cards[4].1 + 51.0,
    ...
    label(p, cards[4].0 + 21.0, cards[4].1 + 101.0, "解像度");
    field(
        p,
        cards[4].0 + 168.0,
        cards[4].1 + 84.0,
    ...
    label(p, cards[4].0 + 21.0, cards[4].1 + 134.0, "フレームレート");
    field(
        p,
        cards[4].0 + 168.0,
        cards[4].1 + 117.0,
    ...
    label(p, cards[4].0 + 21.0, cards[4].1 + 167.0, "品質");
    field(
        p,
        cards[4].0 + 168.0,
        cards[4].1 + 150.0,
    ...
    label(p, cards[5].0 + 21.0, cards[5].1 + 68.0, "プラグインの管理");
    field(
        p,
        cards[5].0 + 285.0,
        cards[5].1 + 51.0,
    ...
    let system = egui::Rect::from_min_size(
        egui::pos2(content.left() + 30.0, content.bottom() - 67.0),
        egui::vec2(content.width() - 60.0, 50.0),
    );
    p.rect(
        system,
        9.0,
    ...
    p.text(
        egui::pos2(system.left() + 21.0, system.center().y),
        egui::Align2::LEFT_CENTER,
        "システム情報",
        egui::FontId::proportional(14.0),
        white,
    ...
    p.text(
        egui::pos2(system.left() + 126.0, system.center().y),
        egui::Align2::LEFT_CENTER,
        "Kagari VFX v0.1.0   │   macOS 14.0   │   Apple M2 Pro   │   メモリ 32 GB   │   Metal 対応",
        egui::FontId::proportional(12.0),
        muted,
    );
    p.rect(
        egui::Rect::from_min_size(
            egui::pos2(system.right() - 178.0, system.top() + 9.0),
    ...
    p.text(
        egui::pos2(system.right() - 99.0, system.center().y),
        egui::Align2::CENTER_CENTER,
        "診断情報をコピー",
        egui::FontId::proportional(12.0),
        white,
...
fn sync_selected_preview(app: &mut KagariApp, ctx: &egui::Context) {
    ...
    let stale = app
        ...
        .unwrap_or(true);
    if stale {
        app.home_preview = crate::ui::embedded_assets::open_image(&path)
            .ok()
            .and_then(|img| {
...
fn draw_reference_topbar(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ui.spacing_mut().item_spacing.x = if ultra_narrow {
                3.0
            } else if narrow {
                5.0
            } else {
...
fn draw_reference_new_project_narrow(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    let right =
        egui::Rect::from_min_size(egui::pos2(top.x + 101.0, top.y), egui::vec2(214.0, 216.0));
    let fill = crate::ui::theme::colors::BG_PANEL_ALT;
    let stroke = egui::Stroke::new(1.0_f32, crate::ui::theme::colors::BORDER_SUBTLE);
    ...
    .enumerate()
    {
        ...
        ui.allocate_new_ui(
            egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
                egui::pos2(left.left() + 10.0, y + 2.0),
                egui::vec2(12.0, 12.0),
            )),
            |icon_ui| {
                crate::ui::icons::render_svg_bytes(
                    icon_ui,
                    &format!("new-project-nav-{index}"),
                    icon,
                    egui::vec2(12.0, 12.0),
                    color,
                );
        ...
        ui.painter().text(
            egui::pos2(left.left() + 25.0, y + 8.0),
            egui::Align2::LEFT_CENTER,
            label,
    ...
    if let Some(id) = reference_texture(app, ctx, "new_project_moon.webp") {
        ui.put(
            egui::Rect::from_min_size(
                egui::pos2(left.left() + 5.0, left.top() + 96.0),
                egui::vec2(90.0, 115.0),
            ),
            egui::Image::new(egui::load::SizedTexture::new(id, egui::vec2(90.0, 115.0)))
                .fit_to_exact_size(egui::vec2(90.0, 115.0)),
        );
    }
    for (index, text) in ["Create.", "Composite.", "Imagine."]
        .into_iter()
        .enumerate()
    {
        ui.painter().text(
            egui::pos2(left.left() + 8.0, left.top() + 158.0 + index as f32 * 11.0),
            egui::Align2::LEFT_CENTER,
            text,
    ...
    ui.painter().text(
        egui::pos2(right.left() + 9.0, right.top() + 13.0),
        egui::Align2::LEFT_CENTER,
    ...
    for (index, (label, value)) in fields.into_iter().enumerate() {
        ...
        let field = egui::Rect::from_min_size(
            egui::pos2(right.left() + 9.0, y + 10.0),
            egui::vec2(188.0, 18.0),
        );
        ui.painter().rect(
            field,
            3.0,
        ...
        if index > 1 {
            ui.painter().text(
                egui::pos2(field.right() - 7.0, field.center().y),
                egui::Align2::CENTER_CENTER,
                "⌄",
                egui::FontId::proportional(7.0),
                colors::TEXT_MUTED,
    ...
    ui.painter().text(
        egui::pos2(right.left() + 9.0, right.bottom() - 41.0),
        egui::Align2::LEFT_CENTER,
        "⌄ Advanced Settings",
        egui::FontId::proportional(6.0),
        colors::TEXT_SECONDARY,
    ...
    let cancel = egui::Rect::from_min_size(
        egui::pos2(right.right() - 113.0, right.bottom() - 26.0),
        egui::vec2(35.0, 19.0),
    );
    ui.painter().rect(
        cancel,
    ...
    let create = egui::Rect::from_min_size(
        egui::pos2(right.right() - 73.0, right.bottom() - 26.0),
        egui::vec2(57.0, 19.0),
    );
    ui.painter()
        .rect(create, 3.0, colors::ACCENT_BLUE, egui::Stroke::NONE);
    ui.painter().text(
        create.center(),
        egui::Align2::CENTER_CENTER,
        "Create Project",
    ...
    ui.painter().line_segment(
        [
            egui::pos2(left.left() + 8.0, left.bottom() - 12.0),
            egui::pos2(left.left() + 22.0, left.bottom() - 12.0),
        ],
...
fn draw_reference_new_project_page(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    ui.painter()
        .rect_filled(ui.max_rect(), 0.0, crate::ui::theme::colors::BG_PANEL_DEEP);
    egui::Frame::none()
        .inner_margin(egui::Margin::symmetric(reference_content_margin(ui), 0.0))
        .show(ui, |ui| {
            draw_reference_topbar(app, ui, ctx);
            ui.add_space(if reference_narrow(ui) {
                8.0
            } else if compact {
                22.0
            } else {
            ...
            let stacked = (!narrow && ui.available_width() < 720.0)
                || (narrow && ui.available_width() < 300.0);
            ui.horizontal_wrapped(|ui| {
                ui.spacing_mut().item_spacing.x = 0.0;
                egui::Frame::none()
                    .fill(crate::ui::theme::colors::BG_PANEL_ALT)
                    ...
                    .show(ui, |ui| {
                        ...
                        if let Some(id) = reference_texture(app, ctx, "recent_project_rift.webp") {
                            ...
                            for (index, line) in ["Create.", "Composite.", "Imagine."]
                                .into_iter()
                                .enumerate()
                            {
                                ui.painter().text(
                                    image_top
                                        + egui::vec2(
                                            4.0,
                                            image_size.y - 34.0 + index as f32 * (text_size + 2.0),
                                        ),
                            ...
                            ui.painter().line_segment(
                                [
                                    egui::pos2(image_top.x + 4.0, line_y),
                                    egui::pos2(
                                        image_top.x + if narrow { 20.0 } else { 38.0 },
                                        line_y,
                                    ),
                                ],
                                egui::Stroke::new(1.0_f32, colors::ACCENT_BLUE),
                ...
                egui::Frame::none()
                    ...
                    .rounding(4.0)
                    .inner_margin(egui::Margin::same(if narrow { 8.0 } else { 16.0 }))
                    .show(ui, |ui| {
                        ...
                        ui.set_width(if narrow {
                            ...
                            (ui.available_width() - 14.0).max(if medium || compact {
                                280.0
                            } else {
                                420.0
                            })
                        ...
                        let mut location = ctx
                            .data_mut(|d| d.get_temp::<String>(location_id))
                            .unwrap_or_else(|| "~/Projects".to_string());
                        ui.add_space(if narrow { 4.0 } else { 8.0 });
                        ui.label(
                            egui::RichText::new("Project Name")
                                .size(if narrow { 8.0 } else { 11.0 })
                                .color(colors::TEXT_SECONDARY),
                        );
                        ui.add(
                        ...
                        ui.horizontal(|ui| {
                            ui.add(egui::TextEdit::singleline(&mut location).desired_width(
                                ui.available_width() - if narrow { 24.0 } else { 30.0 },
                            ));
                            reference_icon_button(
                                ui,
                                "new-project-location",
                                crate::ui::icons::SVG_FOLDER,
                                if narrow { 18.0 } else { 24.0 },
                            );
                        });
                        ui.add_space(if narrow { 4.0 } else { 8.0 });
                        for (label, value) in [
                            ...
                            ui.horizontal(|ui| {
                                ...
                                ui.with_layout(
                                    ...
                                    |ui| {
                                        reference_select_button(
                                            ui,
                                            value,
                                            if narrow {
                                                ui.available_width().min(154.0)
                                            } else {
                                                210.0
                                            },
                                            narrow,
                        ...
                        ui.collapsing("Advanced Settings", |ui| {
                            ui.label(
                                egui::RichText::new("Project defaults and cache options")
                                    .small()
                        ...
                        ui.horizontal(|ui| {
                            if ui
                                .add(
                                    egui::Button::new(
                                        egui::RichText::new("Cancel").size(if narrow {
                                            8.0
                                        } else {
                                            14.0
                                        }),
                                    ...
                                    .min_size(egui::vec2(
                                        if narrow { 42.0 } else { 76.0 },
                                        if narrow { 22.0 } else { 28.0 },
                                    )),
                                )
                                .clicked()
                            {
                                set_home_nav(ui.ctx(), HomeNav::Home);
                            }
                            ui.with_layout(
                                egui::Layout::right_to_left(egui::Align::Center),
                                |ui| {
                                        ...
                                        .add(
                                            egui::Button::new(
                                                egui::RichText::new("Create Project")
                                                    .size(if narrow { 8.0 } else { 14.0 })
                                                    .color(egui::Color32::WHITE),
                                            )
                                            .fill(colors::ACCENT_BLUE)
                                            .min_size(egui::vec2(
                                                if narrow { 68.0 } else { 112.0 },
                                                if narrow { 22.0 } else { 28.0 },
                                            ))
                                            .rounding(5.0),
                                        )
                        ...
                        ctx.data_mut(|d| {
                            ...
                            d.insert_temp(location_id, location);
                        });
                    });
            });
        });
...
fn draw_reference_assets_narrow(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    let center = egui::Rect::from_min_size(
        egui::pos2(left.right() + 5.0, panel_y),
        egui::vec2(151.0, panel_h),
    );
    let detail = egui::Rect::from_min_size(
        egui::pos2(center.right() + 5.0, panel_y),
        egui::vec2(90.0, panel_h),
    );
    let panel_fill = crate::ui::theme::colors::BG_PANEL_ALT;
    let panel_stroke = egui::Stroke::new(1.0_f32, crate::ui::theme::colors::BORDER_SUBTLE);
    ...
    ui.painter().rect(
        center,
        4.0,
        crate::ui::theme::colors::BG_PANEL_DEEP,
        panel_stroke,
    );
    ui.painter().text(
        egui::pos2(left.left() + 6.0, left.top() + 11.0),
        egui::Align2::LEFT_CENTER,
        "Project",
        egui::FontId::proportional(10.0),
        colors::TEXT_PRIMARY,
    ...
    let search = egui::Rect::from_min_size(
        egui::pos2(left.left() + 7.0, left.top() + 27.0),
        egui::vec2(63.0, 18.0),
    );
    ui.painter().rect(
        search,
    ...
    for (index, (label, folder, expanded)) in [
        ("Footage", true, true),
        ("solids", false, false),
        ("city", false, false),
    ...
    .enumerate()
    {
        ...
        if folder {
            ui.allocate_new_ui(
                egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
                    egui::pos2(left.left() + 7.0, y + 2.0),
                    egui::vec2(7.0, 7.0),
                )),
                |icon_ui| {
                    crate::ui::icons::render_svg_bytes(
                        icon_ui,
                        &format!("assets-tree-chevron-{index}"),
                        if expanded {
                            crate::ui::icons::SVG_CHEVRON_DOWN
                        } else {
            ...
            ui.allocate_new_ui(
                egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
                    egui::pos2(left.left() + 16.0, y + 1.0),
                    egui::vec2(10.0, 10.0),
                )),
                |icon_ui| {
                    crate::ui::icons::render_svg_bytes(
                        icon_ui,
                        &format!("assets-tree-folder-{index}"),
                        crate::ui::icons::SVG_FOLDER,
                        egui::vec2(10.0, 10.0),
                        colors::TEXT_SECONDARY,
                    );
            ...
            ui.painter().text(
                egui::pos2(left.left() + 29.0, y + 7.0),
                egui::Align2::LEFT_CENTER,
                label,
                egui::FontId::proportional(7.0),
                colors::TEXT_SECONDARY,
            ...
            ui.allocate_new_ui(
                egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
                    egui::pos2(left.left() + 16.0, y + 3.0),
                    egui::vec2(8.0, 8.0),
                )),
                |icon_ui| {
                    crate::ui::icons::render_svg_bytes(
                        icon_ui,
                        &format!("assets-tree-file-{index}"),
                        crate::ui::icons::SVG_FILE,
                        egui::vec2(8.0, 8.0),
                        colors::TEXT_SECONDARY,
                    );
                },
            ...
            ui.painter().text(
                egui::pos2(left.left() + 27.0, y + 7.0),
                egui::Align2::LEFT_CENTER,
                label,
                egui::FontId::proportional(7.0),
                colors::TEXT_SECONDARY,
    ...
    ui.put(
        egui::Rect::from_min_size(
            egui::pos2(center.left() + 5.0, center.top() + 5.0),
            egui::vec2(48.0, 20.0),
        ),
        egui::Button::new(egui::RichText::new("All Types").size(7.0)),
    );
    ui.put(
        egui::Rect::from_min_size(
            egui::pos2(center.left() + 56.0, center.top() + 5.0),
            egui::vec2(43.0, 20.0),
        ),
        egui::Button::new(egui::RichText::new("Date").size(7.0)),
    );
    let icon_y = center.top() + 15.0;
    let icon_color = colors::TEXT_SECONDARY;
    for (x, kind) in [
        (center.right() - 36.0, 0_u8),
        (center.right() - 24.0, 1_u8),
        (center.right() - 12.0, 2_u8),
    ] {
        let icon = egui::pos2(x, icon_y);
        if kind == 0 {
            for dx in [-3.0_f32, 1.0] {
                for dy in [-3.0_f32, 1.0] {
                    ui.painter().rect(
                        egui::Rect::from_center_size(
                            icon + egui::vec2(dx, dy),
                            egui::vec2(2.0, 2.0),
                        ),
                        0.5,
                        icon_color,
            ...
            ui.painter().line_segment(
                [icon + egui::vec2(-3.0, -3.0), icon + egui::vec2(3.0, -3.0)],
                egui::Stroke::new(1.0_f32, icon_color),
            );
            ui.painter().line_segment(
                [icon + egui::vec2(-2.0, 0.0), icon + egui::vec2(3.0, 0.0)],
                egui::Stroke::new(1.0_f32, icon_color),
            );
            ui.painter().line_segment(
                [icon + egui::vec2(-1.0, 3.0), icon + egui::vec2(3.0, 3.0)],
                egui::Stroke::new(1.0_f32, icon_color),
            );
        } else {
            ui.painter().rect_stroke(
                egui::Rect::from_center_size(icon, egui::vec2(7.0, 7.0)),
                1.0,
                egui::Stroke::new(1.0_f32, icon_color),
            );
            ui.painter().line_segment(
                [icon + egui::vec2(-2.0, 0.0), icon + egui::vec2(2.0, 0.0)],
                egui::Stroke::new(1.0_f32, icon_color),
            );
        }
    }
    ...
    for (index, (asset, name)) in assets.into_iter().enumerate() {
        ...
        let card = egui::Rect::from_min_size(
            egui::pos2(
                center.left() + 5.0 + col as f32 * 48.0,
                center.top() + 34.0 + row as f32 * 64.0,
            ),
            egui::vec2(44.0, 58.0),
        );
        ui.painter().rect(
            card,
            4.0,
            crate::ui::theme::colors::BG_PANEL_RAISED,
            egui::Stroke::new(1.0_f32, crate::ui::theme::colors::BG_ELEVATED),
        );
        if name == "kagari_logo.webp" {
            ui.put(
                card.shrink2(egui::vec2(2.0, 2.0)),
                egui::Image::new(egui::load::SizedTexture::new(
                    reference_texture(app, ctx, asset).unwrap_or_default(),
                    egui::vec2(40.0, 34.0),
                )),
            );
        } else if let Some(id) = reference_texture(app, ctx, asset) {
            ui.put(
                egui::Rect::from_min_size(card.min + egui::vec2(2.0, 2.0), egui::vec2(40.0, 34.0)),
                egui::Image::new(egui::load::SizedTexture::new(id, egui::vec2(40.0, 34.0)))
                    .fit_to_exact_size(egui::vec2(40.0, 34.0)),
            );
        }
        ui.painter().text(
            egui::pos2(card.left() + 2.0, card.bottom() - 12.0),
            egui::Align2::LEFT_CENTER,
            name,
    ...
    ui.painter().text(
        egui::pos2(detail.left() + 5.0, detail.top() + 11.0),
        egui::Align2::LEFT_CENTER,
        "city_01.mp4",
        egui::FontId::proportional(8.0),
        colors::TEXT_PRIMARY,
    ...
    if let Some(id) = reference_texture(app, ctx, "recent_project_citadel.webp") {
        ui.put(
            egui::Rect::from_min_size(
                egui::pos2(detail.left() + 5.0, detail.top() + 21.0),
                egui::vec2(80.0, 43.0),
            ),
            egui::Image::new(egui::load::SizedTexture::new(id, egui::vec2(80.0, 43.0)))
                .fit_to_exact_size(egui::vec2(80.0, 43.0)),
        );
    }
    ui.painter().text(
        egui::pos2(detail.left() + 5.0, detail.top() + 76.0),
        egui::Align2::LEFT_CENTER,
        "Metadata",
        egui::FontId::proportional(7.0),
        colors::TEXT_PRIMARY,
    ...
    .enumerate()
    {
        ...
        ui.painter().text(
            egui::pos2(detail.left() + 5.0, y),
            egui::Align2::LEFT_CENTER,
            label,
            egui::FontId::proportional(5.3),
            colors::TEXT_MUTED,
        );
        ui.painter().text(
            egui::pos2(detail.right() - 5.0, y),
            egui::Align2::RIGHT_CENTER,
            value,
            egui::FontId::proportional(5.3),
            colors::TEXT_SECONDARY,
    ...
    ui.allocate_space(egui::vec2(
        egui::Rect::from_min_size(top, egui::vec2(322.0, panel_h)).width(),
        panel_h,
    ));
}

fn draw_reference_project_browser_page(
    app: &mut KagariApp,
    ui: &mut egui::Ui,
    ctx: &egui::Context,
) {
    ...
    egui::Frame::none()
        .inner_margin(egui::Margin::symmetric(
            if narrow {
                ...
                reference_content_margin(ui)
            },
            0.0,
        ))
        .show(ui, |ui| {
            draw_reference_topbar(app, ui, ctx);
            ui.add_space(if narrow {
                8.0
            } else if compact {
                22.0
            } else {
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui
                        .add(
                            egui::Button::new(
                                egui::RichText::new("＋ 新規プロジェクト").size(if narrow {
                                    9.0
                                } else {
                                    12.0
                                }),
                    ...
                    if ui
                        .add(
                            egui::Button::new(
                                egui::RichText::new("▱ プロジェクトを開く").size(if narrow {
                                    9.0
                                } else {
                                    12.0
                                }),
            ...
            ui.add_space(8.0);
            ui.label(
                egui::RichText::new("保存済みのプロジェクトを管理し、前回の作業を続けます。")
                    .size(if narrow { 9.0 } else { 13.0 })
                    .color(colors::TEXT_SECONDARY),
            );
            ui.add_space(if compact { 12.0 } else { 18.0 });
            reference_search_field(
                ui,
                &mut app.home_search,
                "プロジェクトを検索...",
                ui.available_width().min(480.0),
                if narrow { 26.0 } else { 34.0 },
            );
            ...
            ui.horizontal(|ui| {
                ui.label(
                    egui::RichText::new(format!("最近のプロジェクト  ({})", projects.len()))
                        .size(if narrow { 13.0 } else { 18.0 })
                        .strong(),
                );
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    reference_select_button(
            ...
            if projects.is_empty() {
                ...
                let columns = if narrow {
                    1.0
                } else if ui.available_width() < 820.0 {
                    2.0
                } else {
                    3.0
                };
                ...
                let card_width =
                    ((ui.available_width() - gap * (columns - 1.0)) / columns).max(170.0);
                ui.horizontal_wrapped(|ui| {
                    ui.spacing_mut().item_spacing = egui::vec2(0.0, 0.0);
                    for (index, project) in projects.iter().enumerate() {
                        let card = fixed_card_with_inset(
                            ui,
                            egui::vec2(card_width, if compact { 154.0 } else { 204.0 }),
                            crate::ui::theme::colors::BG_PANEL_RAISED,
                            crate::ui::theme::colors::BG_ELEVATED,
                            6.0,
                            0.0,
                            |ui| {
                                if let Some(id) = reference_texture(
                                    app,
                                    ctx,
                                    [
                                        "recent_project_eclipse.webp",
                                        "recent_project_citadel.webp",
                                        "recent_project_rift.webp",
                                        "recent_project_atlas.webp",
                                    ][index % 4],
                                ) {
                                    reference_cover_image(
                                        ui,
                                        id,
                                        egui::vec2(card_width, if compact { 72.0 } else { 112.0 }),
                                        2.64,
                                    );
                                }
                                ui.add_space(10.0);
                                ...
                                let comp_name = project
                                    .summary
                                    .as_ref()
                                    .and_then(|summary| summary.comp_names.get(summary.active_idx))
                                    .map(String::as_str)
                                    .unwrap_or("Project");
                                ui.horizontal(|ui| {
                                    ui.add_space(12.0);
                        ...
                        if index % columns as usize != columns as usize - 1 {
                            ui.add_space(gap);
                        }
                    }
                });
...
fn draw_reference_projects_page(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    let compact = ui.available_height() < 900.0;
    let bg = crate::ui::theme::colors::BG_PANEL_DEEP;
                    ...
                    ui.allocate_ui(egui::vec2(153.0, 228.0), |ui| { ui.vertical(|ui| {
                        ...
                        ui.horizontal_wrapped(|ui| {
                            ...
                            for (asset, name) in assets {
                                fixed_card_with_inset(ui, egui::vec2(47.0, 49.0), crate::ui::theme::colors::BG_PANEL_RAISED, crate::ui::theme::colors::BG_ELEVATED, 4.0, 0.0, |ui| {
                                    if let Some(id) = reference_texture(app, ctx, asset) {
                ...
                ui.vertical(|ui| {
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        for (asset, name, kind) in asset_cards {
                            ...
                            rendered_assets += 1;
                            let response = fixed_card_with_inset(ui, egui::vec2(card_width, if narrow { 60.0 } else if compact { 124.0 } else { 152.0 }), crate::ui::theme::colors::BG_PANEL_RAISED, crate::ui::theme::colors::BG_ELEVATED, 6.0, if narrow { 0.0 } else { 14.0 }, |ui| {
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.vertical(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new(
                            "Kagari VFX の使い方と制作ワークフローを確認できます。",
                        )
                        .size(if narrow { 9.0 } else { 13.0 })
                        .color(colors::TEXT_SECONDARY),
                    );
            ...
            let cards = [
                (
                    crate::ui::icons::SVG_HOME,
                    "はじめに",
                    "Kagari VFX の画面構成と基本的な考え方",
                ),
                (
                    crate::ui::icons::SVG_LAYERS,
                    "レイヤーと合成",
                    "素材を重ね、調整し、自然な合成を作る",
                ),
                (
                    crate::ui::icons::SVG_EFFECTS,
                    "エフェクト",
                    "Glow、Blur、色補正などの適用方法",
                ),
                (
                    crate::ui::icons::SVG_TRACKING,
                    "トラッキング",
                    "動く素材にマスクやレイヤーを追従させる",
                ),
                (
                ...
                (
                    crate::ui::icons::SVG_SETTINGS,
                    "設定と環境",
                    "自動保存、キャッシュ、GPU設定を管理する",
                ),
            ];
            let cols = if narrow { 1 } else { 2 };
            let gap = if narrow { 8.0 } else { 14.0 };
            let card_w =
                ((ui.available_width() - gap * (cols as f32 - 1.0)) / cols as f32).max(160.0);
            ...
            ui.add_space(12.0);
            ui.label(
                egui::RichText::new("クイックリファレンス")
                    .size(if narrow { 14.0 } else { 18.0 })
                    .strong(),
            );
            ui.add_space(8.0);
            ui.horizontal_wrapped(|ui| {
...
fn documentation_card(
    ...
    crate::ui::icons::render_svg_at(
        ui,
        format!("documentation-card-{title}"),
        icon,
        egui::vec2(
    ...
    ui.put(
        egui::Rect::from_min_max(
            egui::pos2(rect.left() + 58.0, rect.top() + 13.0),
    ...
    ui.put(
        egui::Rect::from_min_max(
            egui::pos2(rect.left() + 58.0, rect.top() + 42.0),
...
fn draw_reference_effects_narrow(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    let center = egui::Rect::from_min_size(
        egui::pos2(left.right() + 2.0, top.y),
        egui::vec2(143.0, 248.0),
    );
    let right = egui::Rect::from_min_size(
        egui::pos2(center.right() + 2.0, top.y),
        egui::vec2(105.0, 248.0),
    );
    let fill = crate::ui::theme::colors::BG_PANEL_ALT;
    let stroke = egui::Stroke::new(1.0_f32, crate::ui::theme::colors::BORDER_SUBTLE);
    ...
    let search = egui::Rect::from_min_size(
        egui::pos2(left.left() + 6.0, left.top() + 27.0),
        egui::vec2(65.0, 20.0),
    );
    ui.painter().rect(
        search,
    ...
    for (index, (icon, label)) in [
        ...
        (crate::ui::icons::SVG_LIGHT, "Distort"),
        (crate::ui::icons::SVG_KEYING, "Keying"),
        (crate::ui::icons::SVG_CLEAN_PLATE, "Blur"),
        (crate::ui::icons::SVG_PARTICLES, "Generate"),
        (crate::ui::icons::SVG_TEMPLATE_TEXT, "Stylize"),
        (crate::ui::icons::SVG_ARROW_RIGHT, "Transition"),
        (crate::ui::icons::SVG_LAYERS, "Utility"),
    ]
    .into_iter()
    .enumerate()
    {
        ...
        if ui
            .interact(
                row,
                egui::Id::new(("reference-effect-narrow-category", index)),
                egui::Sense::click(),
            )
            .clicked()
        {
        ...
        ui.allocate_new_ui(
            egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
                egui::pos2(left.left() + 8.0, y + 3.0),
                egui::vec2(9.0, 9.0),
            )),
            |icon_ui| {
                crate::ui::icons::render_svg_bytes(
                    icon_ui,
                    &format!("effects-category-{index}"),
                    icon,
                    egui::vec2(9.0, 9.0),
                    if active {
                        colors::TEXT_PRIMARY
        ...
        ui.painter().text(
            egui::pos2(left.left() + 21.0, y + 7.0),
            egui::Align2::LEFT_CENTER,
            label,
            egui::FontId::proportional(6.5),
            if active {
    ...
    ui.painter().text(
        egui::pos2(center.left() + 6.0, center.top() + 12.0),
        egui::Align2::LEFT_CENTER,
        "Lumetri Color",
        egui::FontId::proportional(8.0),
        colors::TEXT_PRIMARY,
    ...
    for (offset, label, width) in [(31.0, "Fit", 27.0), (61.0, "Full", 31.0)] {
        let button = egui::Rect::from_min_size(
            egui::pos2(center.left() + offset, center.top() + 5.0),
        ...
        if ui
            .interact(
                button,
                egui::Id::new(("reference-effect-narrow-view", label)),
                egui::Sense::click(),
            )
            .clicked()
        {
            ctx.data_mut(|d| {
                d.insert_temp(
                    egui::Id::new("reference-effect-narrow-view-mode"),
                    label.to_string(),
                )
            });
        }
    ...
    if let Some(id) = reference_texture(app, ctx, "recent_project_rift.webp") {
        ui.put(
            egui::Rect::from_min_size(
                egui::pos2(center.left() + 2.0, center.top() + 44.0),
                egui::vec2(135.0, 170.0),
            ),
            egui::Image::new(egui::load::SizedTexture::new(id, egui::vec2(135.0, 170.0)))
                .fit_to_exact_size(egui::vec2(135.0, 170.0)),
        );
    }
    ui.painter().text(
        egui::pos2(center.left() + 6.0, center.top() + 214.0),
        egui::Align2::LEFT_CENTER,
        "00:00:00:00",
        egui::FontId::proportional(6.5),
        colors::ACCENT_CYAN,
    ...
    ui.painter().text(
        egui::pos2(right.left() + 6.0, right.top() + 11.0),
        egui::Align2::LEFT_CENTER,
        "Recent Controls",
        egui::FontId::proportional(8.0),
        colors::TEXT_PRIMARY,
    ...
    ui.painter().text(
        egui::pos2(right.left() + 6.0, right.top() + 35.0),
        egui::Align2::LEFT_CENTER,
        "Lumetri Color",
        egui::FontId::proportional(6.5),
        colors::TEXT_SECONDARY,
    ...
    let reset = egui::Rect::from_min_size(
        egui::pos2(right.right() - 30.0, right.top() + 25.0),
        egui::vec2(24.0, 16.0),
    );
    ui.painter().text(
        reset.right_center(),
    ...
    if ui
        .interact(
            reset,
            egui::Id::new("reference-effect-narrow-reset"),
            egui::Sense::click(),
        )
        .clicked()
    {
        for (label, default) in [
            ...
            ctx.data_mut(|d| {
                d.insert_temp(egui::Id::new(("reference_effect_value", label)), default)
            });
        }
    }
    for (index, (label, value)) in [
    ...
    .enumerate()
    {
        ...
        ui.painter().text(
            egui::pos2(right.left() + 6.0, y),
            egui::Align2::LEFT_CENTER,
            label,
            egui::FontId::proportional(5.7),
            colors::TEXT_SECONDARY,
        );
        if value.is_empty() {
            ui.painter().text(
                egui::pos2(right.right() - 7.0, y),
                egui::Align2::RIGHT_CENTER,
                "›",
                egui::FontId::proportional(8.0),
                colors::TEXT_MUTED,
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        {
                            let active = selected_category == index;
                            let row_width = ui.available_width().min(if narrow {
                                ui.available_width()
                            } else {
                                154.0
                            });
                                        ...
                                        egui::Frame::none()
                                            ...
                                            .show(ui, |ui| {
                                                ...
                                                ui.horizontal(|ui| {
                                                    crate::ui::icons::render_svg_bytes(
                                                        ui,
                                                        &format!("effect-category-{index}"),
                                                        category_icon,
                                                        egui::vec2(
                                                            if narrow { 8.0 } else { 12.0 },
                                                            if narrow { 8.0 } else { 12.0 },
                                                        ),
                                                        if active {
                                                            colors::TEXT_PRIMARY
                            ...
                            if index == 2 && selected_category == 2 {
                                ...
                                let mut selected_effect = ctx
                                    .data_mut(|d| d.get_temp::<usize>(effect_id))
                                    .unwrap_or(0);
                                ...
                                .enumerate()
                                {
                                    ...
                                    if ui
                                        .add_sized(
                                            [
                                                if narrow { ui.available_width() } else { 146.0 },
                                                if narrow { 18.0 } else { 23.0 },
                                            ],
                                            egui::Button::new(
                                                egui::RichText::new(format!("   {sub_label}"))
                                                    .size(if narrow { 6.0 } else { 11.0 })
                                                    .color(if sub_active {
                                                        colors::TEXT_PRIMARY
                                                    } else {
                                                        colors::TEXT_SECONDARY
                                            ...
                                            .fill(if sub_active {
                                                egui::Color32::from_rgb(22, 52, 84)
                                            } else {
                                                egui::Color32::TRANSPARENT
                                            })
                                            .rounding(3.0),
                ...
                egui::Frame::none()
                    ...
                    .rounding(4.0)
                    .inner_margin(egui::Margin::same(if narrow { 3.0 } else { 12.0 }))
                    .show(ui, |ui| {
                        ui.set_width(if narrow {
                            132.0
                        } else if medium {
                            (ui.available_width() * 0.62).max(260.0)
                        } else {
                            (ui.available_width() * 0.62).max(360.0)
                        });
                        ui.label(egui::RichText::new(selected_effect_name).size(if narrow {
                            10.0
                        } else {
                            16.0
                        }));
                        ...
                        ui.horizontal(|ui| {
                            ...
                            reference_select_button(
                                ui,
                                "Full",
                                if narrow { 34.0 } else { 62.0 },
                                narrow,
                            );
                            ui.add_space(if narrow { 3.0 } else { 8.0 });
                            for (index, icon) in
                                [crate::ui::icons::SVG_GRID, crate::ui::icons::SVG_SORT]
                                    .into_iter()
                                    .enumerate()
                            {
                                crate::ui::icons::render_svg_bytes(
                                    ui,
                                    &format!("effect-view-tool-{index}"),
                                    icon,
                                    egui::vec2(
                                        if narrow { 14.0 } else { 18.0 },
                                        if narrow { 14.0 } else { 18.0 },
                                    ),
                                    colors::TEXT_SECONDARY,
                                );
                        ...
                        if let Some(id) = reference_texture(app, ctx, "recent_project_rift.webp") {
                            let image_size = egui::vec2(
                                ui.available_width(),
                                if narrow || compact { 170.0 } else { 280.0 },
                            );
                            reference_cover_image(ui, id, image_size, 2.64);
                        }
                        ui.add_space(6.0);
                        ui.label(
                            egui::RichText::new("00:00:00:00")
                                .small()
                        ...
                        ui.horizontal(|ui| {
                            ...
                            .enumerate()
                            {
                                crate::ui::icons::render_svg_bytes(
                                    ui,
                                    &format!("effect-transport-{index}"),
                                    icon,
                                    egui::vec2(
                                        if narrow { 12.0 } else { 16.0 },
                                        if narrow { 12.0 } else { 16.0 },
                                    ),
                                    colors::TEXT_SECONDARY,
                                );
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.label(egui::RichText::new("Recent Controls").size(if narrow {
                            9.0
                        } else {
                            14.0
                        }));
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.label(
                                egui::RichText::new("Basic Correction")
                                    .size(if narrow { 7.0 } else { 12.0 })
                                    .strong(),
                            );
                        });
                        ...
                        for (label, default) in [
                            ...
                            let mut value = ctx
                                .data_mut(|d| d.get_temp::<f32>(value_id))
                                .unwrap_or(default);
                            ui.horizontal(|ui| {
                                ui.label(
                                    egui::RichText::new(label)
                                ...
                                ui.with_layout(
                                    ...
                                    |ui| {
                                        ui.add_sized(
                                            [
                                                if narrow { 48.0 } else { 86.0 },
                                                if narrow { 18.0 } else { 22.0 },
                                            ],
                        ...
                        .enumerate()
                        {
                            ...
                            ui.horizontal(|ui| {
                                crate::ui::icons::render_svg_bytes(
                                    ui,
                                    &format!("effect-section-chevron-{index}"),
                                    crate::ui::icons::SVG_CHEVRON_RIGHT,
                                    egui::vec2(
                                        if narrow { 8.0 } else { 12.0 },
                                        if narrow { 8.0 } else { 12.0 },
...
fn draw_reference_render_narrow(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    let settings =
        egui::Rect::from_min_size(egui::pos2(top.x, top.y + 147.0), egui::vec2(163.0, 108.0));
    let progress = egui::Rect::from_min_size(
        egui::pos2(top.x + 167.0, top.y + 147.0),
        egui::vec2(126.0, 108.0),
    );
    let fill = crate::ui::theme::colors::BG_PANEL_ALT;
    let stroke = egui::Stroke::new(1.0_f32, crate::ui::theme::colors::BORDER_SUBTLE);
    for rect in [queue, settings, progress] {
        ui.painter().rect(rect, 4.0, fill, stroke);
    }
    ui.painter().text(
        egui::pos2(queue.left() + 7.0, queue.top() + 12.0),
        egui::Align2::LEFT_CENTER,
        "Render Queue",
        egui::FontId::proportional(10.0),
        colors::TEXT_PRIMARY,
    ...
    let add = egui::Rect::from_min_size(
        egui::pos2(queue.right() - 126.0, queue.top() + 5.0),
        egui::vec2(58.0, 19.0),
    );
    ui.painter().rect(
        add,
    ...
    let start = egui::Rect::from_min_size(
        egui::pos2(queue.right() - 55.0, queue.top() + 5.0),
        egui::vec2(50.0, 19.0),
    );
    ui.painter()
        .rect(start, 3.0, colors::ACCENT_BLUE, egui::Stroke::NONE);
    ui.painter().text(
        start.center(),
        egui::Align2::CENTER_CENTER,
        "Start Render",
    ...
    if ui
        .interact(
            add,
            egui::Id::new("reference-render-narrow-add"),
            egui::Sense::click(),
        )
        .clicked()
    {
    ...
    if ui
        .interact(
            start,
            egui::Id::new("reference-render-narrow-start"),
            egui::Sense::click(),
        )
        .clicked()
    {
    ...
    for (x, label) in [
        (queue.left() + 28.0, "Composition"),
        (queue.left() + 108.0, "Settings"),
        (queue.left() + 201.0, "Output"),
        (queue.right() - 37.0, "Status"),
    ] {
        ui.painter().text(
    ...
    ui.painter().line_segment(
        [
            egui::pos2(queue.left() + 7.0, queue.top() + 31.0),
            egui::pos2(queue.right() - 7.0, queue.top() + 31.0),
        ],
        egui::Stroke::new(1.0_f32, egui::Color32::from_rgb(30, 45, 58)),
    );
    for (index, (asset, name, detail)) in [
        (
            "recent_project_citadel.webp",
            "main_comp",
            "H.264 3840×2160 / 24 fps",
        ),
        (
            "recent_project_rift.webp",
            "teaser",
            "ProRes 422 1920×1080 / 24 fps",
        ),
        (
            "recent_project_atlas.webp",
            "social_vertical",
    ...
    .enumerate()
    {
        ...
        ui.put(
            egui::Rect::from_min_size(egui::pos2(queue.left() + 8.0, y), egui::vec2(10.0, 22.0)),
            egui::Label::new(
        ...
        if let Some(id) = reference_texture(app, ctx, asset) {
            ui.put(
                egui::Rect::from_min_size(
                    egui::pos2(queue.left() + 28.0, y),
                    egui::vec2(31.0, 21.0),
                ),
                egui::Image::new(egui::load::SizedTexture::new(id, egui::vec2(31.0, 21.0)))
                    .fit_to_exact_size(egui::vec2(31.0, 21.0)),
            );
        }
        ui.put(
            egui::Rect::from_min_size(egui::pos2(queue.left() + 65.0, y), egui::vec2(57.0, 20.0)),
            egui::Label::new(egui::RichText::new(name).size(7.0)),
        );
        ui.put(
            egui::Rect::from_min_size(egui::pos2(queue.left() + 124.0, y), egui::vec2(72.0, 20.0)),
            egui::Label::new(
        ...
        ui.painter().text(
            egui::pos2(queue.left() + 201.0, y + 10.0),
            egui::Align2::LEFT_CENTER,
            format!("/renders/{}.mp4", name),
            egui::FontId::proportional(4.5),
            colors::TEXT_SECONDARY,
        );
        ui.put(
            egui::Rect::from_min_size(egui::pos2(queue.right() - 43.0, y), egui::vec2(36.0, 20.0)),
            egui::Label::new(
    ...
    ui.painter().text(
        egui::pos2(settings.left() + 7.0, settings.top() + 12.0),
        egui::Align2::LEFT_CENTER,
        "Render Settings",
        egui::FontId::proportional(8.0),
        colors::TEXT_PRIMARY,
    ...
    .enumerate()
    {
        ...
        let field = egui::Rect::from_min_size(
            egui::pos2(settings.left() + 53.0, y - 7.0),
            egui::vec2(101.0, 14.0),
        );
        ui.painter().rect(
            field,
            2.0,
            egui::Color32::from_rgb(20, 32, 44),
            egui::Stroke::new(1.0_f32, egui::Color32::from_rgb(39, 55, 70)),
        );
        ui.painter().text(
            egui::pos2(field.left() + 5.0, field.center().y),
            egui::Align2::LEFT_CENTER,
            value,
            egui::FontId::proportional(5.5),
            colors::TEXT_PRIMARY,
        ...
        if index < 3 {
            ui.painter().text(
                egui::pos2(field.right() - 6.0, field.center().y),
                egui::Align2::CENTER_CENTER,
                "⌄",
                egui::FontId::proportional(7.0),
                colors::TEXT_MUTED,
    ...
    ui.painter().text(
        egui::pos2(settings.left() + 7.0, settings.bottom() - 12.0),
        egui::Align2::LEFT_CENTER,
    ...
    ui.painter().text(
        egui::pos2(progress.left() + 7.0, progress.top() + 12.0),
        egui::Align2::LEFT_CENTER,
        "Progress",
        egui::FontId::proportional(8.0),
        colors::TEXT_PRIMARY,
    ...
    ui.painter().text(
        egui::pos2(progress.center().x, progress.bottom() - 12.0),
        egui::Align2::CENTER_CENTER,
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ...
            let mut selected = ctx
                .data_mut(|d| d.get_temp::<usize>(selected_id))
                .unwrap_or(0);
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(
                            egui::RichText::new("Render Queue")
                                .size(if narrow { 12.0 } else { 24.0 })
                                .strong(),
                        );
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if ui
                                .add(
                                    egui::Button::new(
                                        egui::RichText::new(if started {
                                            "Rendering..."
                                        } else {
                                            "✦ Start Render"
                                        })
                                        .size(if narrow { 8.0 } else { 12.0 })
                                        .color(egui::Color32::WHITE),
                                    )
                                    .fill(colors::ACCENT_BLUE)
                                    .min_size(egui::vec2(
                                        if narrow { 54.0 } else { 0.0 },
                                        if narrow { 20.0 } else { 0.0 },
                                    ))
                                    .rounding(5.0),
                                )
                                .clicked()
                            ...
                            if ui
                                .add(
                                    egui::Button::new(
                                        egui::RichText::new("+ Add to Queue").size(if narrow {
                                            8.0
                                        } else {
                                            12.0
                                        }),
                    ...
                    ui.horizontal(|ui| {
                        if narrow {
                            ui.spacing_mut().item_spacing.x = 0.0;
                            ui.allocate_ui(egui::vec2(12.0, 18.0), |ui| {
                                ui.label(
                                    egui::RichText::new("#").size(8.0).color(colors::TEXT_MUTED),
                                );
                            });
                            ui.add_space(4.0);
                            ui.allocate_exact_size(egui::vec2(28.0, 18.0), egui::Sense::hover());
                            ui.add_space(5.0);
                            ui.allocate_ui(egui::vec2(48.0, 18.0), |ui| {
                                ui.label(
                                    egui::RichText::new("Composition")
                                        .size(8.0)
                                        .color(colors::TEXT_MUTED),
                                );
                            });
                            ...
                            ui.add_space(5.0);
                            ui.allocate_ui(egui::vec2(54.0, 18.0), |ui| {
                                ui.label(
                    ...
                    for (row_index, name) in queue_names.iter().enumerate() {
                        ...
                        let settings = format!(
                            "{} · {} fps",
                            match app.export_format_preset {
                                1 => "ProRes 422",
                                2 => "PNG Sequence",
                                _ => "H.264",
                            },
                            app.export.export_fps
                        );
                        let output = if app.export.export_output_path.is_empty() {
                            "output.mp4".to_string()
                        } else {
                            app.export.export_output_path.clone()
                        };
                        let status = if app.export.is_exporting
                            && app.export.export_comp_name.as_deref() == Some(name.as_str())
                        {
                            "Rendering"
                        } else {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if narrow {
                                ...
                                ui.allocate_ui(egui::vec2(12.0, 30.0), |ui| {
                                    ui.vertical_centered(|ui| {
                                        ui.label(
                                            egui::RichText::new(index.as_str())
                                                .size(row_font)
                                                .color(colors::TEXT_MUTED),
                                        );
                                    });
                                ...
                                ui.add_space(5.0);
                                if ui
                                    .allocate_ui(egui::vec2(48.0, 30.0), |ui| {
                                        ui.vertical_centered(|ui| {
                                            ui.add(
                                                egui::Label::new(
                                                    egui::RichText::new(name).size(8.0),
                                                )
                                                .sense(egui::Sense::click()),
                                            )
                                        })
                                ...
                                ui.allocate_ui(egui::vec2(58.0, 30.0), |ui| {
                                    ui.vertical(|ui| {
                                        ui.label(
                                            egui::RichText::new(
                                                settings_parts.next().unwrap_or(&settings),
                                            )
                                            .size(row_font)
                                            .color(colors::TEXT_SECONDARY),
                                        );
                                ...
                                ui.add_space(4.0);
                                let row_status = if started && selected == row_index {
                                    "Rendering"
                                } else {
                                    status
                                };
                                ...
                                crate::ui::icons::render_svg_bytes(
                                    ui,
                                    &format!("render-row-more-{index}"),
                                    crate::ui::icons::SVG_MORE,
                                    egui::vec2(10.0, 10.0),
                                    colors::TEXT_MUTED,
                                );
                            } else {
                        ...
                        if row_index + 1 < queue_names.len() {
                            ui.separator();
                        }
                    }
                });
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                egui::Frame::none()
                    ...
                    .rounding(4.0)
                    .inner_margin(egui::Margin::same(if narrow { 6.0 } else { 12.0 }))
                    .show(ui, |ui| {
                        ui.set_width(if narrow {
                            141.0
                        } else if medium {
                            (ui.available_width() * 0.54).max(300.0)
                        } else {
                            (ui.available_width() * 0.54).max(360.0)
                        });
                        ui.label(egui::RichText::new("Render Settings").size(if narrow {
                            10.0
                        } else {
                            14.0
                        }));
                        ...
                        for (label, default, options) in [
                            ("Format", "H.264", ["H.264", "ProRes 422", "PNG Sequence"]),
                            (
                                "Resolution",
                                "3840 × 2160 (4K UHD)",
                                [
                            ...
                            ("Frame Rate", "24 fps", ["24 fps", "30 fps", "60 fps"]),
                        ] {
                                ...
                                ui.with_layout(
                                    ...
                                    |ui| {
                                        egui::ComboBox::from_id_salt(id)
                                            ...
                                            .width(if narrow { 66.0 } else { 220.0 })
                                            .show_ui(ui, |ui| {
                            ...
                            match label {
                                "Format" => {
                                    ...
                                    app.export_codec_idx = if app.export_format_preset == 1 {
                                        1
                                    } else if app.export_format_preset == 2 {
                                        3
                                    } else {
                                        0
                                    };
                                ...
                                "Resolution" => {
                                    app.export_resolution_scale = if selected_value.contains("1920")
                                    {
                                        1
                                    } else if selected_value.contains("1080 × 1920") {
                                        2
                                    } else {
                                        0
                                    };
                                    let scale = match app.export_resolution_scale {
                                        1 => 0.5,
                                        2 => 0.5,
                                        _ => 1.0,
                                    };
                                    ctx.data_mut(|d| {
                                        d.insert_temp(egui::Id::new("ae_export_res_scale"), scale)
                                    });
                                }
                                "Frame Rate" => {
                                    app.export.export_fps = selected_value
                                        .split_whitespace()
                                        .next()
                        ...
                        let mut output_path = ctx
                            .data_mut(|d| d.get_temp::<String>(path_id))
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.with_layout(
                                ...
                                |ui| {
                                    ui.add_sized(
                                        [
                                            if narrow { 38.0 } else { 190.0 },
                                            if narrow { 18.0 } else { 24.0 },
                                        ],
                                        egui::TextEdit::singleline(&mut output_path),
                                    );
                                    reference_icon_button(
                                        ui,
                        ...
                        let mut open_folder = ctx
                            .data_mut(|d| d.get_temp::<bool>(open_folder_id))
                            .unwrap_or(true);
                        ui.checkbox(
                            &mut open_folder,
                            egui::RichText::new("Open output folder when complete")
                                .size(if narrow { 8.0 } else { 11.0 }),
                        );
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.vertical_centered(|ui| {
                            ui.label(egui::RichText::new("Progress").size(if narrow {
                                9.0
                            } else {
                                14.0
                            }));
                            ...
                            let (ring_rect, _) = ui.allocate_exact_size(
                                ...
                                for index in 0..segments {
                                    let a0 = start_angle
                                        + (end_angle - start_angle) * index as f32
                                            / segments as f32;
                                    let a1 = start_angle
                                        + (end_angle - start_angle) * (index + 1) as f32
                                            / segments as f32;
                                    ui.painter().line_segment(
                                        [
                                        ...
                                        egui::Stroke::new(4.0_f32, colors::ACCENT_BLUE),
                                    );
                                }
                            }
                            ui.painter().text(
                            ...
                            ui.label(
                                egui::RichText::new(if app.export.is_exporting {
                                    format!(
                                        "Rendering {}",
                                        app.export
                                            .export_comp_name
                                ...
                                } else if app.export.export_status.is_some() {
                                    "Render finished".to_string()
                                } else {
                                    "No render in progress".to_string()
                                })
                                .small()
                                .color(colors::TEXT_MUTED),
                            );
...
fn draw_reference_templates_narrow(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    let stroke = egui::Stroke::new(1.0_f32, crate::ui::theme::colors::BORDER_SUBTLE);
    ui.painter()
        .rect(panel, 4.0, crate::ui::theme::colors::BG_PANEL_ALT, stroke);
    ui.painter().text(
        egui::pos2(panel.left() + 5.0, panel.top() + 13.0),
        egui::Align2::LEFT_CENTER,
        "Project Templates",
        egui::FontId::proportional(9.0),
        colors::TEXT_PRIMARY,
    ...
    ui.painter().text(
        egui::pos2(panel.left() + 6.0, panel.top() + 37.0),
        egui::Align2::LEFT_CENTER,
        "⌕  Search templates...",
        egui::FontId::proportional(6.0),
        colors::TEXT_SECONDARY,
    ...
    .enumerate()
    {
        ...
        ui.put(
            egui::Rect::from_min_size(
                egui::pos2(x, panel.top() + 27.0),
                egui::vec2(widths[index], 20.0),
            ),
            egui::Button::new(egui::RichText::new(label).size(5.5)).fill(if index == 0 {
                colors::ACCENT_BLUE
            } else {
                egui::Color32::TRANSPARENT
            }),
    ...
    let cards = [
        (
            "templates/cinematic_title.webp",
            "Cinematic Title",
            "Film & Video",
        ),
        (
            "templates/logo_reveal.webp",
            "Logo Reveal",
            "Motion Graphics",
        ),
        ("templates/hud_interface.webp", "HUD Interface", "VFX"),
        (
            "templates/product_showcase.webp",
            "Product Showcase",
            "Motion Graphics",
        ),
        ("templates/vfx_breakdown.webp", "VFX Breakdown", "VFX"),
        (
            "templates/social_media_reel.webp",
            "Social Media Reel",
            "Social Media",
        ),
        (
            "templates/text_animation.webp",
            "Text Animation",
            "Motion Graphics",
        ),
    ...
    for (index, (asset, title, category)) in cards.into_iter().enumerate() {
        let card = egui::Rect::from_min_size(
            egui::pos2(
                panel.left() + 6.0 + (index % 4) as f32 * 77.0,
                panel.top() + 52.0 + (index / 4) as f32 * 76.0,
            ),
            egui::vec2(64.0, 70.0),
        );
        ui.painter().rect(
        ...
        if let Some(id) = reference_texture(app, ctx, asset) {
            ui.put(
                egui::Rect::from_min_size(
                    egui::pos2(card.left() + 3.0, card.top() + 3.0),
                    egui::vec2(58.0, 36.0),
                ),
                egui::Image::new(egui::load::SizedTexture::new(id, egui::vec2(58.0, 36.0)))
                    .fit_to_exact_size(egui::vec2(58.0, 36.0)),
            );
        }
        ui.put(
            egui::Rect::from_min_size(
                egui::pos2(card.left() + 3.0, card.top() + 42.0),
                egui::vec2(58.0, 13.0),
            ),
            egui::Label::new(egui::RichText::new(title).size(5.8).strong()),
        );
        ui.put(
            egui::Rect::from_min_size(
                egui::pos2(card.left() + 3.0, card.top() + 55.0),
                egui::vec2(58.0, 11.0),
            ),
            egui::Label::new(
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.label(
                        egui::RichText::new("Project Templates")
                            .size(if narrow { 10.0 } else { 18.0 })
                            .strong(),
                    );
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        reference_search_field(
                            ...
                            "Search templates...",
                            search_width,
                            if narrow { 20.0 } else { 28.0 },
                        );
                        ui.add_space(6.0);
                        let filter_id = egui::Id::new("reference_template_filter");
                        let mut selected_filter = ctx
                            .data_mut(|d| d.get_temp::<usize>(filter_id))
                            .unwrap_or(0);
                        for (index, label) in [
                            "All",
                            "Film & Video",
                        ...
                        .enumerate()
                        {
                            ...
                            let filter_width = if narrow {
                                [22.0, 42.0, 52.0, 22.0, 42.0][index]
                            } else {
                                0.0
                            };
                            let filter = egui::Button::new(
                                egui::RichText::new(label)
                                    .size(if narrow { 7.0 } else { 11.0 })
                                    .color(if active {
                                        colors::TEXT_PRIMARY
                    ...
                    ui.add_space(12.0);
                    let selected_filter = ctx.data_mut(|d| {
                        d.get_temp::<usize>(egui::Id::new("reference_template_filter"))
                            .unwrap_or(0)
                    });
                    let search_query = app.home_search.clone();
                    draw_reference_template_catalog(
                        app,
                        ctx,
                        ui,
                        compact,
...
fn draw_reference_settings_narrow(ui: &mut egui::Ui) {
    ...
    let content =
        egui::Rect::from_min_size(egui::pos2(top.x + 101.0, top.y), egui::vec2(193.0, 219.0));
    let fill = crate::ui::theme::colors::BG_PANEL_ALT;
    let stroke = egui::Stroke::new(1.0_f32, crate::ui::theme::colors::BORDER_SUBTLE);
    ui.painter().rect(left, 4.0, fill, stroke);
    ...
    text(
        egui::pos2(content.left() + 8.0, content.top() + 14.0),
        "General",
        10.0,
        colors::TEXT_PRIMARY,
    );
    text(
        egui::pos2(content.left() + 8.0, content.top() + 39.0),
        "Language",
        6.0,
        colors::TEXT_SECONDARY,
    );
    field(
        egui::Rect::from_min_size(
            egui::pos2(content.left() + 62.0, content.top() + 28.0),
    ...
    .enumerate()
    {
        ...
        ui.painter().rect_filled(
            egui::Rect::from_min_size(
                egui::pos2(content.left() + 8.0, y - 4.0),
                egui::vec2(8.0, 8.0),
            ),
            1.0,
            if checked {
                ...
                egui::Color32::from_rgb(39, 53, 67)
            },
        );
        text(
            egui::pos2(content.left() + 21.0, y),
            label,
            6.0,
            colors::TEXT_SECONDARY,
        );
    ...
    text(
        egui::pos2(content.left() + 8.0, content.top() + 108.0),
        "Project Defaults",
        7.0,
        colors::TEXT_PRIMARY,
    );
    ...
    .enumerate()
    {
        ...
        field(
            egui::Rect::from_min_size(
                egui::pos2(content.left() + 62.0, y - 8.0),
    ...
    text(
        egui::pos2(content.left() + 8.0, content.top() + 180.0),
        "File Handling",
        7.0,
        colors::TEXT_PRIMARY,
    );
    for (index, label) in ["Remember last project on startup", "Show startup screen"]
        .into_iter()
        .enumerate()
    {
    ...
    let reset = egui::Rect::from_min_size(
        egui::pos2(content.right() - 72.0, content.bottom() - 25.0),
        egui::vec2(65.0, 18.0),
    );
    ui.painter().rect(
        reset,
...
fn draw_reference_settings_page(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    let compact = ui.available_height() < 900.0;
    let narrow = reference_narrow(ui);
    let settings_margin = if narrow {
        8.0
    } else {
        reference_content_margin(ui)
    };
    let prefs_id = egui::Id::new("reference_settings_prefs");
    let initial_prefs = ctx.data_mut(|d| {
        d.get_temp::<crate::ui::preferences_dialog::Prefs>(prefs_id)
            .unwrap_or_else(crate::ui::preferences_dialog::load)
    });
    let mut prefs = initial_prefs.clone();
    ui.painter()
        .rect_filled(ui.max_rect(), 0.0, crate::ui::theme::colors::BG_PANEL_DEEP);
    egui::Frame::none()
        .inner_margin(egui::Margin::symmetric(settings_margin, 0.0))
        .show(ui, |ui| {
            draw_reference_topbar(app, ui, ctx);
            ui.add_space(if reference_narrow(ui) {
                8.0
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                egui::Frame::none()
                    ...
                    .rounding(4.0)
                    .inner_margin(egui::Margin::same(if narrow { 2.0 } else { 8.0 }))
                    .show(ui, |ui| {
                        ui.set_width(if narrow {
                            88.0
                        } else if medium {
                        ...
                        let mut selected = ctx
                            .data_mut(|d| d.get_temp::<usize>(section_id))
                            .unwrap_or(0)
                            .min(7);
                        for (index, (icon, label)) in [
                            (crate::ui::icons::SVG_SETTINGS, "General"),
                ...
                ui.add_space(if narrow { 6.0 } else { 12.0 });
                let selected_section = ctx.data_mut(|d| {
                    d.get_temp::<usize>(egui::Id::new("reference_settings_section"))
                        .unwrap_or(0)
                        .min(7)
                });
                let section_names = [
                ...
                egui::Frame::none()
                    ...
                    .rounding(4.0)
                    .inner_margin(egui::Margin::same(if narrow { 2.0 } else { 16.0 }))
                    .show(ui, |ui| {
                        ui.set_width(if narrow {
                            204.0
                        } else {
                            (ui.available_width() - 12.0).max(if medium { 320.0 } else { 420.0 })
                        });
                        ui.label(
                            egui::RichText::new(section_names[selected_section])
                                .size(if narrow { 12.0 } else { 16.0 })
                                .strong(),
                        );
                        if !narrow {
                            ui.label(
                                egui::RichText::new(section_descriptions[selected_section])
                                    .small()
                                    .color(colors::TEXT_MUTED),
                            );
                        }
                        if selected_section != 0 {
                            ...
                            match selected_section {
                                1 => {
                                    ui.label(
                                        egui::RichText::new("Preview Quality")
                                            .size(if narrow { 9.0 } else { 12.0 })
                                            .color(colors::TEXT_SECONDARY),
                                    );
                                    ui.checkbox(
                                        &mut prefs.adaptive_preview,
                                        "Adaptive preview quality",
                                    );
                                    ui.label(
                                        egui::RichText::new(format!(
                                            "Frame cache budget: {} MB",
                                            prefs.cache_mb
                                        ))
                                        .size(if narrow { 8.0 } else { 11.0 })
                                        .color(colors::TEXT_SECONDARY),
                                    ...
                                    ui.add(
                                        egui::Slider::new(&mut prefs.cache_mb, 128..=2048)
                                            .step_by(64.0)
                                            .suffix(" MB"),
                                    );
                                }
                                2 => {
                                    ui.label(
                                        egui::RichText::new("Disk Cache")
                                            .size(if narrow { 9.0 } else { 12.0 })
                                            .color(colors::TEXT_SECONDARY),
                                    );
                                    ui.label(
                                        egui::RichText::new(format!(
                                            "Maximum disk cache: {} GB",
                                            prefs.disk_cache_gb
                                        ))
                                        .size(if narrow { 8.0 } else { 11.0 })
                                        .color(colors::TEXT_SECONDARY),
                                    ...
                                    ui.add(
                                        egui::Slider::new(&mut prefs.disk_cache_gb, 10..=500)
                                            .suffix(" GB"),
                                    );
                                }
                                4 => {
                                    ui.label(
                                        egui::RichText::new("Automatic Save")
                                            .size(if narrow { 9.0 } else { 12.0 })
                                            .color(colors::TEXT_SECONDARY),
                                    );
                                    ui.label(
                                    ...
                                    ui.add(
                                        egui::Slider::new(&mut prefs.autosave_secs, 5..=600)
                                            .suffix(" s"),
                                    );
                                }
                                5 => {
                                    ...
                                    ui.label(
                                        egui::RichText::new("Dark theme")
                                            .size(if narrow { 8.0 } else { 11.0 })
                                            .color(colors::TEXT_PRIMARY),
                                    );
                                    ui.label(
                                        egui::RichText::new(
                                            "Kagari VFX uses the dark production workspace.",
                                        )
                                        .small()
                                        .color(colors::TEXT_MUTED),
                                    );
                            ...
                            let mut updates = ctx
                                .data_mut(|d| d.get_temp::<bool>(updates_id))
                                .unwrap_or(true);
                            let mut anonymous = ctx
                                .data_mut(|d| d.get_temp::<bool>(anonymous_id))
                                .unwrap_or(false);
                            let mut startup = ctx
                                .data_mut(|d| d.get_temp::<bool>(startup_id))
                                .unwrap_or(true);
                            let mut splash = ctx
                                .data_mut(|d| d.get_temp::<bool>(splash_id))
                                .unwrap_or(true);
                            ui.checkbox(
                                &mut updates,
                                egui::RichText::new("Check for updates automatically")
                                    .size(if narrow { 8.0 } else { 11.0 }),
                            );
                            ui.checkbox(
                                &mut anonymous,
                                egui::RichText::new("Send anonymous usage data").size(if narrow {
                                    8.0
                                } else {
                                    11.0
                                }),
                            ...
                            ui.add_space(if narrow { 5.0 } else { 12.0 });
                            ui.label(
                                egui::RichText::new("Project Defaults")
                                    .size(if narrow { 9.0 } else { 12.0 })
                                    .color(colors::TEXT_PRIMARY),
                            );
                            ui.add_space(if narrow { 3.0 } else { 6.0 });
                            ...
                            ui.checkbox(
                                &mut startup,
                                egui::RichText::new("Remember last project on startup")
                                    .size(if narrow { 8.0 } else { 11.0 }),
                            );
                            ui.checkbox(
                                &mut splash,
                                egui::RichText::new("Show startup screen").size(if narrow {
                                    8.0
                                } else {
                                    11.0
                                }),
                                ...
                                ui.with_layout(
                                    ...
                                    |ui| {
                                        if ui
                                            ...
                                            .clicked()
                                        {
                                            crate::ui::preferences_dialog::reset_to_defaults(app);
                                        }
                                    },
                                );
                            });
                            ctx.data_mut(|d| {
                                d.insert_temp(updates_id, updates);
                                d.insert_temp(anonymous_id, anonymous);
                                d.insert_temp(startup_id, startup);
                                d.insert_temp(splash_id, splash);
                            });
                        }
                    });
            });
...
fn draw_reference_template_catalog(
    app: &mut KagariApp,
    ctx: &egui::Context,
    ui: &mut egui::Ui,
    compact: bool,
    ...
    let columns = 4.0;
    let width = ((ui.available_width() - gap * (columns - 1.0)) / columns).max(if narrow {
        54.0
    } else {
        130.0
    });
    ...
    let visible_template_count = cards
        .iter()
        .filter(|(_, title, category)| matches_filter(title, category))
        .count();
                ...
                |ui| {
                    ui.vertical(|ui| {
                        ...
                        ui.horizontal_centered(|ui| {
                            let image_size = egui::vec2(
                                (width - if narrow { 4.0 } else { 28.0 }).max(10.0),
                                icon_height,
                            );
                            if let Some(id) = reference_texture(app, ctx, asset) {
    ...
    if visible_template_count == 0 {
        ...
        ui.vertical_centered(|ui| {
            ui.label(
                egui::RichText::new("No templates found")
                    .size(14.0)
                    .color(colors::TEXT_PRIMARY),
            );
            ui.label(
                egui::RichText::new("Try another category or search term.")
                    .small()
                    .color(colors::TEXT_MUTED),
            );
        });
...
fn draw_reference_nav(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    for (nav, icon, label) in [
        ...
        crate::ui::icons::render_svg_at(
            ...
            egui::pos2(
                row_rect.left() + if compact_sidebar { 12.0 } else { 18.0 },
        ...
        ui.put(
            egui::Rect::from_min_max(
                ...
                egui::pos2(row_rect.right() - 10.0, row_rect.bottom()),
            ),
    ...
    ui.with_layout(egui::Layout::bottom_up(egui::Align::Min), |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.painter().line_segment(
                ...
                egui::Stroke::new(1.0_f32, egui::Color32::from_rgb(47, 63, 78)),
            );
        });
        ui.add_space(10.0);
    });
...
fn reference_texture(
    ...
    let path = if name == "render_preview.webp" {
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("assets/studio/render_preview.webp")
    } else if let Some(asset) = name.strip_prefix("templates/") {
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
...
fn asset_metadata_for_name(name: &str) -> &'static [(&'static str, &'static str)] {
    match name {
        ...
        "glass_texture.png" => &[
            ("Frame Size", "4096 × 4096"),
            ("Format", "PNG"),
            ("Color", "RGBA"),
            ("File Size", "12 MB"),
        ],
        "logo.png" | "kagari_logo.webp" => &[
            ("Frame Size", "1254 × 1254"),
            ("Format", "WebP"),
            ("Color", "RGBA"),
            ("File Size", "264 KB"),
        ],
        ...
        "stroke_01.mov" | "light_leak.mov" | "particles.mov" => &[
            ("Frame Rate", "24 fps"),
...
fn reference_action_button(
    ...
    let (rect, response) = ui.allocate_exact_size(egui::vec2(width, height), egui::Sense::click());
    let fill = if accent {
        egui::Color32::from_rgb(24, 92, 180)
    } else {
        egui::Color32::from_rgb(28, 39, 51)
    };
    ui.painter().rect_filled(rect, 6.0, fill);
    let stroke = if accent {
        colors::ACCENT_BLUE
    } else {
        egui::Color32::from_rgb(45, 62, 79)
    };
    ui.painter()
        .rect_stroke(rect, 6.0, egui::Stroke::new(1.0_f32, stroke));
    ...
    inner.vertical(|ui| {
        ui.label(
            egui::RichText::new(title)
                .size(if height >= 80.0 { 14.0 } else { 11.0 })
                .strong(),
        );
        if !subtitle.is_empty() {
            ui.label(
                egui::RichText::new(subtitle)
                    .size(if height >= 80.0 { 12.0 } else { 9.0 })
                    .color(colors::TEXT_SECONDARY),
            );
        }
    });
    inner.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
        crate::ui::icons::render_svg_bytes(
            ui,
            &format!("{title}-arrow"),
            crate::ui::icons::SVG_ARROW_RIGHT,
            egui::vec2(15.0, 15.0),
            egui::Color32::from_rgb(190, 204, 226),
        );
    });
    response
}
...
fn reference_cta_button(ui: &mut egui::Ui, title: &str, compact: bool) -> egui::Response {
    ...
    let (rect, response) = ui.allocate_exact_size(egui::vec2(width, height), egui::Sense::click());
    ui.painter()
        .rect_filled(rect, 6.0, egui::Color32::from_rgb(22, 118, 239));
    let mut inner = ui.new_child(
        egui::UiBuilder::new()
        ...
        |ui| {
            let clicked = ui
                .scope(|ui| {
                    ...
                    ui.add(
                        egui::Button::new(egui::RichText::new(label).color(colors::TEXT_SECONDARY))
                ...
                .clicked();
            ui.add_space(2.0);
            let arrow_clicked = crate::ui::icons::render_svg_bytes(
                ui,
                "view-all-arrow",
                crate::ui::icons::SVG_ARROW_RIGHT,
                egui::vec2(15.0, 15.0),
            ...
            ui.horizontal(|ui| {
                ...
                ui.add(
                    egui::TextEdit::singleline(value)
                        ...
                        .frame(false)
                        .desired_width((inner_width - 13.0).max(1.0)),
                );
            });
        });
}

fn reference_asset_search_field(
    ui: &mut egui::Ui,
    value: &mut String,
    hint: &str,
    width: f32,
                    ...
                    egui::TextEdit::singleline(value)
                        ...
                        .desired_width(
                            (inner_width
                                - icon_size
                                - filter_size
                                - if narrow { 6.0 } else { 16.0 })
                            .max(1.0),
                        ),
                );
                crate::ui::icons::render_svg_bytes(
                    ui,
                    "reference-asset-search-filter",
                    crate::ui::icons::SVG_CHEVRON_DOWN,
                    egui::vec2(filter_size, filter_size),
                    colors::TEXT_SECONDARY,
                );
            });
        });
...
fn reference_select_button(
    ...
    let (rect, response) = ui.allocate_exact_size(egui::vec2(width, height), egui::Sense::click());
    ui.painter()
        .rect_filled(rect, 4.0, egui::Color32::from_rgb(24, 35, 47));
    ui.painter().rect_stroke(
        rect,
        4.0,
        egui::Stroke::new(1.0_f32, crate::ui::theme::colors::BORDER_MEDIUM),
    ...
    child.label(
        egui::RichText::new(label)
            .size(if compact { 8.5 } else { 11.0 })
            .color(colors::TEXT_PRIMARY),
    );
    child.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
        crate::ui::icons::render_svg_bytes(
            ui,
            "reference-select-chevron",
            crate::ui::icons::SVG_CHEVRON_DOWN,
            egui::vec2(11.0, 11.0),
            colors::TEXT_SECONDARY,
        );
...
fn reference_contain_image(
    ...
    ui.painter()
        .rect_filled(rect, 5.0, egui::Color32::from_rgb(7, 12, 18));
...
fn draw_reference_home(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    let base_first_row_height = if mobile {
        360.0
    } else if compact {
        200.0
    } else {
        ...
        .show(ui, |ui| {
            ...
            ui.allocate_ui_with_layout(
                ...
                |ui| {
                    ...
                    let desired_search_width: f32 = if mobile {
                        220.0
                    } else if compact {
                        300.0
                    } else {
                    ...
                    let search_width =
                        desired_search_width.min((ui.available_width() - 52.0).max(1.0));
                    ui.allocate_ui(egui::vec2(search_width, 46.0), |ui| {
                        egui::Frame::none()
                            .fill(egui::Color32::from_rgb(14, 23, 32))
                            .stroke(egui::Stroke::new(
                                1.0_f32,
                                egui::Color32::from_rgb(49, 67, 84),
                            ))
                            .rounding(7.0)
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    ui.add(
                                        egui::TextEdit::singleline(&mut app.home_search)
                                            .hint_text("Search projects, templates...")
                                            .font(egui::FontId::proportional(search_font_size))
                                            .frame(false)
                                            .desired_width(
                                                (search_width - if mobile { 48.0 } else { 54.0 })
                                                    .max(1.0),
                                            ),
                                    );
                                });
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.label(
                            egui::RichText::new("Pick up where you left off.")
                                .size(if mobile { 20.0 } else { 28.0 })
                                .strong()
                                .color(egui::Color32::from_rgb(188, 201, 232)),
                        );
                    });
                    ui.add_space(10.0);
                    let row_width = ui.available_width();
                    ...
                    let draw_home_first_row = |ui: &mut egui::Ui| {
                        ...
                        fixed_card(
                            ui,
                            egui::vec2(width, first_row_height),
                            crate::ui::theme::colors::BG_PANEL_RAISED,
                            crate::ui::theme::colors::BG_ELEVATED,
                            8.0,
                            |ui| {
                                ui.horizontal(|ui| {
                                    ui.label(
                                        egui::RichText::new("Continue Working").size(20.0).strong(),
                                    );
                                    ui.with_layout(
                                        egui::Layout::right_to_left(egui::Align::Center),
                                        |ui| {
                                            crate::ui::icons::render_svg_bytes(
                                                ...
                                                crate::ui::icons::SVG_MORE,
                                                egui::vec2(18.0, 18.0),
                                                egui::Color32::from_rgb(170, 188, 216),
                                            );
                                        },
                                    );
                                });
                                ui.add_space(if compact { 8.0 } else { 12.0 });
                                draw_reference_continue_body(
                                    app,
                                    ui,
                                    ctx,
                                    compact,
                        ...
                        let start_height = if stacked && row_width < 560.0 {
                            214.0
                        } else {
                            first_row_height
                        };
                        fixed_card(
                            ui,
                            egui::vec2(start_width, start_height),
                            crate::ui::theme::colors::BG_PANEL_RAISED,
                            crate::ui::theme::colors::BG_ELEVATED,
                            8.0,
                            |ui| {
                                ...
                                if ui.available_width() < 260.0 {
                                    for (icon, title, subtitle, action) in [
                                        ...
                                        (
                                            crate::ui::icons::SVG_OPEN_FOLDER,
                                            "Open Project",
                                            "Browse files",
                                            1_u8,
                                        ),
                                        (
                                            crate::ui::icons::SVG_COMPOSITION,
                                            "New Composition",
                                            "Create a comp",
                                            2_u8,
                                        ),
                                        ...
                                        let clicked = reference_action_button(
                                            ui,
                                            icon,
                                            title,
                                            subtitle,
                                            action == 0,
                                            action_height.min(62.0),
                                        )
                                        .clicked();
                                        if clicked {
                                            match action {
                                    ...
                                    ui.horizontal(|ui| {
                                        let button_width =
                                            ((ui.available_width() - action_gap) / 2.0).max(72.0);
                                        if ui
                                            .allocate_ui(
                                                egui::vec2(button_width, action_height),
                                                |ui| {
                                            ...
                                            .clicked()
                                        {
                                            set_home_nav(ui.ctx(), HomeNav::NewProject);
                                        }
...
fn draw_reference_continue_preview(
    ...
    if let Some(id) = reference_texture(app, ctx, "continue_working_preview.webp") {
        ...
        egui::Frame::none()
            .stroke(egui::Stroke::new(
                1.0_f32,
                egui::Color32::from_rgb(54, 72, 91),
            ))
            .rounding(5.0)
            .show(ui, |ui| {
...
fn draw_reference_continue_details(
    ...
    ui.label(
        egui::RichText::new("Eclipse")
            .size(if compact { 18.0 } else { 20.0 })
            .strong(),
    );
    ui.add_space(8.0);
    ui.horizontal(|ui| {
    ...
    if reference_cta_button(ui, "Resume Composition", compact).clicked() {
        if let Some(top) = recent_project_entries(ctx)
            .into_iter()
...
fn landing_nav_row(
    ...
    if active {
        let active_rect =
            egui::Rect::from_min_max(egui::pos2(rect.left() + 17.0, rect.top()), rect.max);
        ui.painter()
            .rect_filled(active_rect, 0.0, crate::ui::theme::colors::BG_PANEL_DARK);
        ui.painter().rect_filled(
            egui::Rect::from_min_size(
                egui::pos2(rect.left() + 17.0, rect.top()),
    ...
    ui.allocate_new_ui(egui::UiBuilder::new().max_rect(label_rect), |label_ui| {
        label_ui.with_layout(
            egui::Layout::left_to_right(egui::Align::Center),
            |label_ui| {
                label_ui.add(
                    egui::Label::new(egui::RichText::new(label).size(16.0).color(if active {
                        colors::TEXT_PRIMARY
                    } else {
                        crate::ui::theme::colors::TEXT_SECONDARY_BRIGHT
                    }))
...
fn draw_landing_sidebar(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    let current = home_nav(ctx);
    ui.painter()
        .rect_filled(ui.max_rect(), 0.0, egui::Color32::from_rgb(17, 25, 31));
    ui.add_space(17.0);
    ui.horizontal(|ui| {
        ui.add_space(29.0);
        if app.home_banner.is_none() {
    ...
    let settings_rect = egui::Rect::from_min_size(
        egui::pos2(
            side.left(),
            side.bottom() - if short_sidebar { 126.0 } else { 180.0 },
        ),
    ...
    let exit_rect = egui::Rect::from_min_size(
        egui::pos2(
            side.left(),
            side.bottom() - if short_sidebar { 70.0 } else { 124.0 },
        ),
        egui::vec2(side.width(), 56.0),
    );
    ui.allocate_new_ui(
        egui::UiBuilder::new().max_rect(settings_rect),
        |settings_ui| {
            landing_nav_row(
                settings_ui,
                ctx,
...
fn landing_button_rect(
    ...
    ui.painter().rect(
        ...
        if accent {
            ...
            egui::Color32::from_rgb(25, 33, 40)
        },
        egui::Stroke::new(
            1.0_f32,
            if accent {
                egui::Color32::from_rgb(255, 144, 58)
            } else {
                egui::Color32::from_rgb(54, 68, 80)
            },
        ),
    );
    ui.allocate_new_ui(
        egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
            egui::pos2(rect.left() + 44.0, rect.center().y - 14.0),
            egui::vec2(28.0, 28.0),
        )),
        |icon_ui| {
            crate::ui::icons::render_svg_bytes(
                icon_ui,
                &format!("landing-button-{label}"),
                icon,
                egui::vec2(28.0, 28.0),
                egui::Color32::WHITE,
            );
    ...
    ui.painter().text(
        egui::pos2(rect.left() + 91.0, rect.center().y),
        egui::Align2::LEFT_CENTER,
        label,
        egui::FontId::proportional(16.0),
        colors::TEXT_PRIMARY,
...
fn draw_landing_hero(ui: &mut egui::Ui, rect: egui::Rect) {
    ...
    for (delta, width, color) in [
        ...
        let points = curve(
            ...
            egui::pos2(
                rect.left() + rect.width() * 0.64 - delta,
                rect.top() + 132.0,
            ),
        ...
        painter.add(egui::Shape::line(
            points,
            egui::Stroke::new(width, color.linear_multiply(0.72)),
        ));
    }
    painter.text(
        egui::pos2(rect.right() - 2.0, rect.top() + 39.0),
        egui::Align2::RIGHT_CENTER,
        "Create. Composite. Illuminate.",
        egui::FontId::proportional(14.0),
        egui::Color32::from_rgb(159, 168, 181),
    );
}
...
fn draw_landing_home(app: &mut KagariApp, ui: &mut egui::Ui, _ctx: &egui::Context) {
    ...
    let hero = egui::Rect::from_min_size(
        ...
        egui::vec2(
            (content.width() - margin - right_margin).max(1.0),
            if mobile { 321.0 } else { 334.0 },
        ),
    );
    draw_landing_hero(ui, hero);
    ui.painter().text(
        egui::pos2(hero.left(), hero.top() + 158.0),
        egui::Align2::LEFT_CENTER,
        "映像に、想像を重ねる。",
        egui::FontId::proportional(if mobile { 29.0 } else { 43.0 }),
        colors::TEXT_PRIMARY,
    );
    ui.painter().text(
        egui::pos2(hero.left(), hero.top() + 211.0),
        egui::Align2::LEFT_CENTER,
        "ノードでも、レイヤーでも、自在に。あなたのビジョンを、より遠くへ。",
        egui::FontId::proportional(if mobile { 13.0 } else { 18.0 }),
        egui::Color32::from_rgb(168, 180, 193),
    );
    let button_width = if mobile { 220.0 } else { 266.0 };
    let button_top = hero.top() + 254.0;
    let first_button = egui::Rect::from_min_size(
        egui::pos2(hero.left(), button_top),
        egui::vec2(button_width, 62.0),
    );
    let second_button = egui::Rect::from_min_size(
        egui::pos2(hero.left() + button_width + 23.0, button_top),
        egui::vec2(button_width, 62.0),
    );
    if landing_button_rect(
        ui,
        first_button,
        "新規プロジェクト",
        crate::ui::icons::SVG_FILE_PLUS,
    ...
    ui.painter().line_segment(
        [
            egui::pos2(content.left() + margin, section_top),
            egui::pos2(content.right() - right_margin, section_top),
        ],
        egui::Stroke::new(1.0_f32, egui::Color32::from_rgb(48, 61, 71)),
    );
    ui.painter().text(
        egui::pos2(content.left() + margin, section_top + 36.0),
        egui::Align2::LEFT_CENTER,
        "最近のプロジェクト",
    ...
    let recent_height = if compact_mobile {
        196.0
    } else if mobile {
        188.0
    } else {
    ...
    let recent = egui::Rect::from_min_size(
        egui::pos2(content.left() + margin, section_top + 53.0),
        egui::vec2(content.width() - margin - right_margin, recent_height),
    );
    ui.painter()
        .rect_filled(recent, 7.0, egui::Color32::from_rgb(14, 22, 28));
    ...
    ui.allocate_new_ui(
        egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
            egui::pos2(
                empty_center - 22.0,
                recent.top() + if compact_mobile { 22.0 } else { 33.0 },
            ),
            egui::vec2(44.0, 44.0),
        )),
        |empty_ui| {
            crate::ui::icons::render_svg_bytes(
                empty_ui,
                "landing-empty-folder",
                crate::ui::icons::SVG_FOLDER,
                egui::vec2(44.0, 44.0),
                egui::Color32::from_rgb(161, 176, 194),
            );
        },
    );
    ui.painter().text(
        egui::pos2(
            empty_center,
            recent.top() + if compact_mobile { 94.0 } else { 108.0 },
        ),
        egui::Align2::CENTER_CENTER,
        "まだプロジェクトはありません",
    ...
    ui.painter().text(
        egui::pos2(
            empty_center,
            recent.top() + if compact_mobile { 119.0 } else { 140.0 },
        ),
        egui::Align2::CENTER_CENTER,
        "新規プロジェクトを作成して、はじめましょう。",
        egui::FontId::proportional(14.0),
    ...
    let empty_button = egui::Rect::from_min_size(
        egui::pos2(
            recent.center().x - 114.0,
            if compact_mobile {
                recent.bottom() - 62.0
            } else {
                recent.center().y + 52.0
            },
        ),
    ...
    let col_gap = 44.0;
    let left_width = if mobile {
        lower_width
    } else {
        (lower_width - col_gap) * 0.567
    };
    ui.painter().text(
        egui::pos2(content.left() + margin, lower_top),
        egui::Align2::LEFT_CENTER,
        "ニュース",
        egui::FontId::proportional(19.0),
        colors::TEXT_PRIMARY,
    );
    let news = egui::Rect::from_min_size(
        egui::pos2(content.left() + margin, lower_top + 17.0),
        egui::vec2(left_width, 164.0),
    );
    ui.painter().rect(
        news,
        8.0,
    ...
    for (index, (label, date)) in [
        ...
        ("はじめに：基本操作ガイド", "2025/08/20"),
        ("サンプルプロジェクトを追加", "2025/08/10"),
    ]
    .into_iter()
    .enumerate()
    {
        ...
        ui.painter().circle_filled(
            egui::pos2(news.left() + 28.0, y),
            7.0,
            if index == 0 {
                egui::Color32::from_rgb(255, 136, 33)
            } else {
                egui::Color32::from_rgb(145, 160, 174)
            },
        );
        ui.painter().text(
            egui::pos2(news.left() + 53.0, y),
            egui::Align2::LEFT_CENTER,
            label,
            egui::FontId::proportional(14.0),
            colors::TEXT_PRIMARY,
        ...
        ui.painter().text(
            egui::pos2(news.right() - 31.0, y),
            egui::Align2::RIGHT_CENTER,
            date,
            egui::FontId::proportional(13.0),
            egui::Color32::from_rgb(157, 170, 183),
        );
        if index < 2 {
            ui.painter().line_segment(
                [
    ...
    if !mobile {
        ...
        ui.painter().text(
            egui::pos2(resources_left, lower_top),
            egui::Align2::LEFT_CENTER,
            "リソース",
            egui::FontId::proportional(19.0),
            colors::TEXT_PRIMARY,
        );
        let resources = egui::Rect::from_min_size(
            egui::pos2(resources_left, lower_top + 17.0),
            egui::vec2(lower_width - left_width - col_gap, 164.0),
        );
        ui.painter().rect(
            resources,
            8.0,
        ...
        for (index, (icon, label)) in [
            ...
            (crate::ui::icons::SVG_PLAY_CIRCLE, "チュートリアルを見る"),
            (crate::ui::icons::SVG_CHAT, "コミュニティ"),
        ]
        .into_iter()
        .enumerate()
        {
            ...
            ui.allocate_new_ui(
                egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
                    egui::pos2(resources.left() + 19.0, y - 13.0),
                    egui::vec2(27.0, 27.0),
                )),
                |icon_ui| {
                    crate::ui::icons::render_svg_bytes(
                        icon_ui,
                        &format!("landing-resource-{index}"),
                        icon,
                        egui::vec2(27.0, 27.0),
                        egui::Color32::from_rgb(200, 212, 224),
                    );
                },
            );
            ui.painter().text(
                egui::pos2(resources.left() + 72.0, y),
                egui::Align2::LEFT_CENTER,
                label,
                egui::FontId::proportional(14.0),
                colors::TEXT_PRIMARY,
            ...
            ui.painter().text(
                egui::pos2(resources.right() - 24.0, y),
                egui::Align2::CENTER_CENTER,
                "↗",
                egui::FontId::proportional(22.0),
                egui::Color32::from_rgb(189, 202, 214),
            );
            if index < 2 {
                ui.painter().line_segment(
                    [
...
fn draw_reference_project_row(
        ...
        let empty = fixed_card_with_inset(
            ...
            |ui| {
                ui.vertical_centered(|ui| {
                    ...
                    ui.add_space(if compact { 8.0 } else { 12.0 });
                    ui.label(
                        egui::RichText::new("まだプロジェクトはありません")
                            .size(if compact { 14.0 } else { 19.0 })
                            .strong(),
                    );
                    ui.label(
                        egui::RichText::new("新規プロジェクトを作成して、はじめましょう。")
                            .size(if compact { 10.0 } else { 14.0 })
                            .color(colors::TEXT_SECONDARY),
                    );
    ...
    let columns = if available_width < 680.0 {
        1.0
    } else if available_width < 1040.0 {
        2.0
    } else {
        4.0
    };
    let card_gap = if columns == 1.0 {
        0.0
    } else if compact {
        12.0
    } else {
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        for (card_index, project) in projects.iter().take(4).enumerate() {
            ...
            let age = project
                ...
                .map(fmt_age)
                .unwrap_or_else(|| "Not found".to_string());
            let response = fixed_card_with_inset(
                ui,
                ...
                |ui| {
                    if let Some(id) = reference_texture(app, ctx, asset) {
                        reference_cover_image(
                            ui,
                            id,
                            egui::vec2(card_width, image_height),
                            316.0 / 121.0,
                        );
                    }
                    ui.add_space(14.0);
...
fn draw_reference_templates(ui: &mut egui::Ui, compact: bool, show_header: bool) {
    ...
    let illustration_height = if narrow {
        34.0
    } else if compact {
        38.0
    } else {
    ...
    ui.spacing_mut().item_spacing.x = 0.0;
    ui.horizontal_wrapped(|ui| {
        for (index, (icon, title, desc)) in [
            (
                crate::ui::icons::SVG_KEYING,
                "Keying Setup",
            ...
            (
                crate::ui::icons::SVG_CLEAN_PLATE,
                "Clean Plate Setup",
                "Remove objects",
            ),
            (
                crate::ui::icons::SVG_PARTICLES,
                "Particle Composite",
                "Sparks, smoke, fire",
            ),
            (
                crate::ui::icons::SVG_TITLE,
                "Title Animation",
                "Motion graphics",
            ),
            (
                crate::ui::icons::SVG_SCREEN,
                "Screen Replacement",
            ...
            let response = fixed_card_with_inset(
                ...
                |ui| {
                    ui.vertical(|ui| {
                        ...
                        ui.horizontal(|ui| {
                            ui.add_space(
                                ((ui.available_width() - illustration_width) * 0.5).max(0.0),
                            );
                            crate::ui::icons::render_svg_bytes(
                                ui,
                                title,
                                icon,
                                egui::vec2(illustration_width, illustration_height),
                                egui::Color32::WHITE,
                            );
                        });
                        ui.add_space(4.0);
...
fn draw_nav(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    for (nav, icon, label) in [
        ...
        let resp = ui.horizontal(|ui| {
            ...
            ui.selectable_label(
                current == nav,
                egui::RichText::new(label).small().color(if current == nav {
                    colors::TEXT_PRIMARY
                } else {
                    colors::TEXT_SECONDARY
                }),
...
fn draw_resume(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    frame.show(ui, |ui| {
        ui.horizontal(|ui| {
            ...
            ui.painter()
                .rect_stroke(tile, 3.0, egui::Stroke::new(1.0_f32, colors::BORDER_MEDIUM));
            ui.painter().text(
                tile.center(),
                egui::Align2::CENTER_CENTER,
                "PROJECT",
            ...
            ui.vertical(|ui| {
                ...
                let sub = match top.modified {
                    Some(m) => format!("Edited {}  ·  {}", fmt_age(m), top.path.display()),
                    None => top.path.display().to_string(),
                };
                ui.label(egui::RichText::new(sub).small().color(colors::TEXT_MUTED));
            });
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
...
fn draw_recent_projects(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context, limit: usize) {
    ui.horizontal(|ui| {
...
fn draw_project_rows(
    ...
    for entry in entries.iter() {
        ...
        ui.horizontal(|ui| {
            let star_label = if is_starred(&entry.path) {
                "★"
            } else {
                "☆"
            };
...
fn draw_templates_view(app: &mut KagariApp, ui: &mut egui::Ui) {
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            crate::ui::theme::draw_section_header_svg(
                ui,
                "Templates",
                crate::ui::icons::SVG_LAYERS,
            );
...
fn draw_session(app: &mut KagariApp, ui: &mut egui::Ui) {
    ...
    for (name, state, good) in [
        ...
        (
            "GPU Preview",
            if gpu { "Active" } else { "CPU fallback" },
            gpu,
        ),
        ("Autosave", "On", true),
    ] {
        ui.horizontal(|ui| {
            ui.label(egui::RichText::new("●").small().color(if good {
                colors::ACCENT_GREEN
            } else {
...
fn task_summary(app: &KagariApp) -> (String, egui::Color32, bool) {
    ...
    if app.export.is_exporting {
        ...
        return (format!("Rendering… {}%", pct), colors::ACCENT_GREEN, true);
    }
    if app.render_queue_items.is_empty() {
        return ("No background tasks".to_string(), colors::TEXT_MUTED, false);
    }
...
fn draw_status_strip(app: &mut KagariApp, ui: &mut egui::Ui) {
    ui.horizontal(|ui| {
        ...
        ui.label(egui::RichText::new("●").small().color(color));
        ui.label(egui::RichText::new(text).small().color(if active {
            colors::TEXT_PRIMARY
        } else {
            colors::TEXT_MUTED
        }));
...
mod tests {
    ...
    fn narrow_render_add_button_updates_real_queue_state() {
        ...
        assert!(app
            .render_item_status
            .values()
            .all(|status| matches!(status, crate::app_state::QueueItemStatus::Queued)));
    }
    ...
    fn narrow_effects_category_click_updates_real_selection_state() {
        ...
        for y in (90..190).step_by(2) {
            ...
            if ctx.data(|data| {
                data.get_temp::<usize>(egui::Id::new("reference-effect-narrow-category"))
            }) == Some(3)
            {
                break;
            }
        ...
        assert_eq!(
            ctx.data(
                |data| data.get_temp::<usize>(egui::Id::new("reference-effect-narrow-category"))
            ),
            Some(3)
        );
    }
