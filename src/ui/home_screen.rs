...
fn draw_export_target(app: &mut KagariApp, ctx: &egui::Context) {
    ...
    egui::CentralPanel::default().frame(egui::Frame::none().fill(egui::Color32::from_rgb(11,19,25))).show(ctx, |ui| {
        ...
        let rows=[("形式","MP4 (H.264)"),("コーデック","H.264 (AVC)"),("解像度","1920 × 1080 (フルHD)"),("フレームレート","30 fps"),("ビットレート","高品質（20 Mbps）")]; let field_x=settings.left()+if width>=1200.0{214.0}else{settings.width()*0.35}; for (i,(lab,val)) in rows.into_iter().enumerate(){let yy=settings.top()+72.0+i as f32*38.0; ui.painter().text(egui::pos2(settings.left()+20.0,yy+11.0),egui::Align2::LEFT_CENTER,lab,egui::FontId::proportional(if width>=1200.0{14.0}else{10.0}),colors::TEXT_SECONDARY); ui.painter().rect(egui::Rect::from_min_size(egui::pos2(field_x,yy-5.0),egui::vec2(settings.right()-field_x-20.0,32.0)),5.0,egui::Color32::from_rgb(20,32,41),stroke); ui.painter().text(egui::pos2(field_x+12.0,yy+11.0),egui::Align2::LEFT_CENTER,val,egui::FontId::proportional(if width>=1200.0{13.0}else{9.0}),colors::TEXT_PRIMARY);} let checkbox = |y: f32, label: &str, checked: bool| { let box_rect=egui::Rect::from_min_size(egui::pos2(settings.left()+214.0,y-9.0),egui::vec2(18.0,18.0)); ui.painter().rect(box_rect,3.0,if checked{egui::Color32::from_rgb(255,103,24)}else{egui::Color32::TRANSPARENT},stroke); if checked { ui.painter().text(box_rect.center(),egui::Align2::CENTER_CENTER,"✓",egui::FontId::proportional(12.0),egui::Color32::WHITE); } ui.painter().text(egui::pos2(settings.left()+244.0,y),egui::Align2::LEFT_CENTER,label,egui::FontId::proportional(if width>=1200.0{13.0}else{9.0}),colors::TEXT_SECONDARY); }; checkbox(settings.top()+264.0,"アルファチャンネルを含める（透過）",true); checkbox(settings.top()+295.0,"HDR（Rec. 2100 / HLG）",false); ui.painter().line_segment([egui::pos2(settings.left()+20.0,settings.top()+319.0),egui::pos2(settings.right()-20.0,settings.top()+319.0)],stroke); for (i,(lab,val)) in [("保存先","/Users/username/Movies/Kagari VFX"),("ファイル名","Sample Project")].into_iter().enumerate(){let yy=settings.top()+338.0+i as f32*38.0; ui.painter().text(egui::pos2(settings.left()+20.0,yy+11.0),egui::Align2::LEFT_CENTER,lab,egui::FontId::proportional(if width>=1200.0{14.0}else{10.0}),colors::TEXT_SECONDARY); ui.painter().rect(egui::Rect::from_min_size(egui::pos2(field_x,yy-5.0),egui::vec2(settings.right()-field_x-20.0,32.0)),5.0,egui::Color32::from_rgb(20,32,41),stroke); ui.painter().text(egui::pos2(field_x+12.0,yy+11.0),egui::Align2::LEFT_CENTER,val,egui::FontId::proportional(if width>=1200.0{13.0}else{9.0}),colors::TEXT_PRIMARY);}
        let export_action = egui::Rect::from_min_size(egui::pos2(settings.left()+20.0,settings.bottom()-60.0),egui::vec2(settings.width()-40.0,47.0)); ui.painter().rect(export_action,6.0,egui::Color32::from_rgb(255,103,24),egui::Stroke::NONE); crate::ui::icons::render_svg_at(ui,"export-start-icon".to_string(),crate::ui::icons::SVG_EXPORT,egui::vec2(18.0,18.0),egui::Color32::WHITE,egui::pos2(settings.center().x-84.0,settings.bottom()-45.0)); ui.painter().text(egui::pos2(settings.center().x+10.0,settings.bottom()-36.0),egui::Align2::CENTER_CENTER,"書き出し開始",egui::FontId::proportional(if width>=1200.0{15.0}else{11.0}),egui::Color32::WHITE); if ui.interact(export_action,egui::Id::new("reference-export-start"),egui::Sense::click()).clicked(){let comp_name=app.history.current().active_composition().name.clone(); crate::ui::export_dialog::start_comp_export(app,ctx,&comp_name);}
        let queue_y = if width >= 1200.0 { r.top() + 582.0 } else { settings.bottom() + 14.0 };
        let queue = egui::Rect::from_min_size(
            egui::pos2(title_x, queue_y),
            egui::vec2(main.width() - margin * 2.0, (height - queue_y - r.top() - 34.0).max(150.0)),
        );
        let p = ui.painter();
        ...
        p.text(egui::pos2(queue.left() + 20.0, queue.top() + queue_title_y), egui::Align2::LEFT_CENTER, "レンダーキュー（3）", egui::FontId::proportional(if width >= 1200.0 { 17.0 } else { 13.0 }), colors::TEXT_PRIMARY);
        if width >= 1200.0 {
            let headers = [(20.0, "#"), (130.0, "プロジェクト名"), (310.0, "形式"), (445.0, "解像度"), (565.0, "フレームレート"), (660.0, "状態"), (810.0, "進行状況"), (1035.0, "完了時の動作")];
            for (offset, label) in headers {
                p.text(egui::pos2(queue.left() + offset, queue.top() + queue_header_y), egui::Align2::LEFT_CENTER, label, egui::FontId::proportional(11.0), colors::TEXT_MUTED);
            }
            let rows = [
                ("Sample Project", "MP4 (H.264)", "1920 × 1080", "30 fps", "レンダリング中", 0.68_f32, "68 %", "render_preview.webp"),
                ("City Overview", "MOV (ProRes 422)", "3840 × 2160", "24 fps", "待機中", 0.0_f32, "0 %", "recent_project_citadel.webp"),
                ("Particles Shot", "MP4 (H.265)", "1920 × 1080", "60 fps", "完了", 1.0_f32, "100 %", "recent_project_atlas.webp"),
            ];
            for (i, (name, format, resolution, fps, status, progress, percent, asset)) in rows.into_iter().enumerate() {
                let yy = queue.top() + queue_row_start + i as f32 * queue_row_step;
                p.line_segment([egui::pos2(queue.left() + 18.0, yy + 22.0), egui::pos2(queue.right() - 18.0, yy + 22.0)], egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE));
                p.text(egui::pos2(queue.left() + 20.0, yy), egui::Align2::LEFT_CENTER, format!("{}", i + 1), egui::FontId::proportional(11.0), colors::TEXT_SECONDARY);
                if let Some(id) = reference_texture(app, ctx, asset) {
                    let thumb = egui::Rect::from_min_size(egui::pos2(queue.left() + 44.0, yy - 16.0), egui::vec2(66.0, 36.0));
                    p.image(id, thumb, egui::Rect::from_min_max(egui::pos2(0.0, 0.0), egui::pos2(1.0, 1.0)), egui::Color32::WHITE);
                    p.rect_stroke(thumb, 4.0, egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE));
                }
                p.text(egui::pos2(queue.left() + 130.0, yy), egui::Align2::LEFT_CENTER, name, egui::FontId::proportional(11.0), colors::TEXT_PRIMARY);
                p.text(egui::pos2(queue.left() + 310.0, yy), egui::Align2::LEFT_CENTER, format, egui::FontId::proportional(10.0), colors::TEXT_SECONDARY);
                p.text(egui::pos2(queue.left() + 445.0, yy), egui::Align2::LEFT_CENTER, resolution, egui::FontId::proportional(10.0), colors::TEXT_SECONDARY);
                p.text(egui::pos2(queue.left() + 565.0, yy), egui::Align2::LEFT_CENTER, fps, egui::FontId::proportional(10.0), colors::TEXT_SECONDARY);
                p.circle_filled(egui::pos2(queue.left() + 665.0, yy), 6.0, if i == 0 { colors::ACCENT_CYAN } else if i == 2 { colors::ACCENT_GREEN } else { egui::Color32::YELLOW });
                p.text(egui::pos2(queue.left() + 682.0, yy), egui::Align2::LEFT_CENTER, status, egui::FontId::proportional(10.0), if i == 0 { colors::ACCENT_CYAN } else if i == 2 { colors::ACCENT_GREEN } else { colors::TEXT_SECONDARY });
                let progress_rect = egui::Rect::from_min_size(egui::pos2(queue.left() + 810.0, yy - 4.0), egui::vec2(140.0, 8.0));
                p.rect_filled(progress_rect, 4.0, colors::BORDER_MEDIUM);
                p.rect_filled(egui::Rect::from_min_size(progress_rect.min, egui::vec2(progress_rect.width() * progress, progress_rect.height())), 4.0, if i == 2 { colors::ACCENT_GREEN } else { colors::ACCENT_CYAN });
                p.text(egui::pos2(queue.left() + 960.0, yy), egui::Align2::LEFT_CENTER, percent, egui::FontId::proportional(10.0), colors::TEXT_SECONDARY);
                p.text(egui::pos2(queue.left() + 1035.0, yy), egui::Align2::LEFT_CENTER, if i == 2 { "サウンドで通知" } else { "通知" }, egui::FontId::proportional(10.0), colors::TEXT_PRIMARY);
            }
        } else {
            let cols = ["#", "プロジェクト名", "形式", "解像度", "フレームレート", "状態", "進行状況", "完了時の動作"];
            for (i, c) in cols.into_iter().enumerate() {
                p.text(egui::pos2(queue.left() + 20.0 + i as f32 * (queue.width() / 8.0), queue.top() + queue_header_y), egui::Align2::LEFT_CENTER, c, egui::FontId::proportional(8.0), colors::TEXT_MUTED);
            }
            for (i, (name, status)) in [("Sample Project", "レンダリング中"), ("City Overview", "待機中"), ("Particles Shot", "完了")].into_iter().enumerate() {
                let yy = queue.top() + queue_row_start + i as f32 * queue_row_step;
                p.line_segment([egui::pos2(queue.left() + 18.0, yy + 22.0), egui::pos2(queue.right() - 18.0, yy + 22.0)], egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE));
                p.text(egui::pos2(queue.left() + 20.0, yy), egui::Align2::LEFT_CENTER, format!("{}", i + 1), egui::FontId::proportional(11.0), colors::TEXT_SECONDARY);
                p.text(egui::pos2(queue.left() + queue.width() * 0.10, yy), egui::Align2::LEFT_CENTER, name, egui::FontId::proportional(11.0), colors::TEXT_PRIMARY);
                p.text(egui::pos2(queue.left() + queue.width() * 0.50, yy), egui::Align2::LEFT_CENTER, status, egui::FontId::proportional(11.0), if i == 0 { colors::ACCENT_CYAN } else if i == 2 { colors::ACCENT_GREEN } else { colors::TEXT_SECONDARY });
            }
        }
    });
}
...
fn draw_reference_documentation_page(app: &mut KagariApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    egui::Frame::none().inner_margin(egui::Margin::symmetric(if narrow { 8.0 } else { reference_content_margin(ui) }, 0.0)).show(ui, |ui| {
        ...
        let cards = [
            ...
            (crate::ui::icons::SVG_EFFECTS, "エフェクト", "Glow、Blur、色補正などの適用方法"),
...
fn reference_texture(app: &mut KagariApp, ctx: &egui::Context, name: &str) -> Option<egui::TextureId> {
    ...
    let path = if name == "render_preview.webp" {
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("assets/studio/render_preview.webp")
