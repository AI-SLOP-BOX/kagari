...
fn draw_export_target(app: &mut KagariApp, ctx: &egui::Context) {
    ...
    egui::CentralPanel::default().frame(egui::Frame::none().fill(egui::Color32::from_rgb(11,19,25))).show(ctx, |ui| {
        ...
        if !narrow { ui.painter().text(egui::pos2(32.0,r.bottom()-27.0),egui::Align2::LEFT_CENTER,"Kagari VFX   v0.1.0",egui::FontId::proportional(13.0),colors::TEXT_SECONDARY); }
...
fn draw_settings_target_content(ui: &mut egui::Ui, content: egui::Rect) {
    ...
    let cards = [(content.left() + 30.0, content.top() + 103.0, 528.0, 250.0, "インターフェース"), (content.left() + 571.0, content.top() + 103.0, 547.0, 250.0, "プロジェクトと保存"), (content.left() + 30.0, content.top() + 365.0, 528.0, 257.0, "パフォーマンス"), (content.left() + 571.0, content.top() + 365.0, 547.0, 257.0, "メディアとタイムライン"), (content.left() + 30.0, content.top() + 633.0, 528.0, 184.0, "書き出しの既定設定"), (content.left() + 571.0, content.top() + 633.0, 547.0, 184.0, "プラグイン")];
    ...
    label(p, cards[0].0 + 21.0, cards[0].1 + 68.0, "テーマ"); field(p, cards[0].0 + 190.0, cards[0].1 + 51.0, 313.0, "システム          ダーク          ライト");
    ...
    label(p, cards[0].0 + 21.0, cards[0].1 + 154.0, "言語"); field(p, cards[0].0 + 190.0, cards[0].1 + 137.0, 313.0, "日本語"); label(p, cards[0].0 + 21.0, cards[0].1 + 198.0, "UIのスケール"); field(p, cards[0].0 + 190.0, cards[0].1 + 181.0, 313.0, "100%（推奨）");
    label(p, cards[1].0 + 21.0, cards[1].1 + 68.0, "自動保存の間隔"); field(p, cards[1].0 + 261.0, cards[1].1 + 51.0, 263.0, "5分"); label(p, cards[1].0 + 21.0, cards[1].1 + 108.0, "自動保存ファイルの保持数"); field(p, cards[1].0 + 261.0, cards[1].1 + 91.0, 263.0, "10個"); label(p, cards[1].0 + 21.0, cards[1].1 + 152.0, "起動時に前回のプロジェクトを開く"); label(p, cards[1].0 + 21.0, cards[1].1 + 196.0, "プロジェクトのバックアップを作成"); label(p, cards[1].0 + 21.0, cards[1].1 + 240.0, "プロジェクトの既定の保存先"); field(p, cards[1].0 + 221.0, cards[1].1 + 223.0, 303.0, "/Users/username/Kagari VFX/Projects");
    ...
    switch_pill(p, cards[1].0 + 479.0, cards[1].1 + 118.0);
    switch_pill(p, cards[1].0 + 479.0, cards[1].1 + 162.0);
    label(p, cards[2].0 + 21.0, cards[2].1 + 68.0, "GPUアクセラレーション"); label(p, cards[2].0 + 21.0, cards[2].1 + 108.0, "使用するGPU"); field(p, cards[2].0 + 190.0, cards[2].1 + 91.0, 313.0, "自動選択（推奨）"); label(p, cards[2].0 + 21.0, cards[2].1 + 152.0, "メモリの使用上限"); p.line_segment([egui::pos2(cards[2].0 + 190.0, cards[2].1 + 140.0), egui::pos2(cards[2].0 + 438.0, cards[2].1 + 140.0)], egui::Stroke::new(5.0_f32, egui::Color32::from_rgb(255,107,22))); p.circle_filled(egui::pos2(cards[2].0 + 352.0, cards[2].1 + 140.0), 8.0, white); label(p, cards[2].0 + 453.0, cards[2].1 + 140.0, "75%"); label(p, cards[2].0 + 21.0, cards[2].1 + 194.0, "キャッシュフォルダ"); field(p, cards[2].0 + 164.0, cards[2].1 + 177.0, 339.0, "/Users/username/Library/Caches/Kagari VFX");
    label(p, cards[3].0 + 21.0, cards[3].1 + 68.0, "プロキシメディアを自動生成"); label(p, cards[3].0 + 21.0, cards[3].1 + 108.0, "プロキシ解像度"); field(p, cards[3].0 + 290.0, cards[3].1 + 91.0, 234.0, "1/2（推奨）"); label(p, cards[3].0 + 21.0, cards[3].1 + 152.0, "タイムラインの既定のフレームレート"); field(p, cards[3].0 + 290.0, cards[3].1 + 135.0, 234.0, "30 fps"); label(p, cards[3].0 + 21.0, cards[3].1 + 196.0, "オーディオの波形を生成"); label(p, cards[3].0 + 21.0, cards[3].1 + 240.0, "タイムラインのスクロール動作"); field(p, cards[3].0 + 290.0, cards[3].1 + 223.0, 234.0, "スムーズスクロール");
    switch_pill(p, cards[3].0 + 479.0, cards[3].1 + 45.0);
    switch_pill(p, cards[3].0 + 479.0, cards[3].1 + 177.0);
    label(p, cards[4].0 + 21.0, cards[4].1 + 68.0, "フォーマット"); field(p, cards[4].0 + 168.0, cards[4].1 + 51.0, 335.0, "H.264 (MP4)"); label(p, cards[4].0 + 21.0, cards[4].1 + 101.0, "解像度"); field(p, cards[4].0 + 168.0, cards[4].1 + 84.0, 335.0, "1920 × 1080（フルHD）"); label(p, cards[4].0 + 21.0, cards[4].1 + 134.0, "フレームレート"); field(p, cards[4].0 + 168.0, cards[4].1 + 117.0, 335.0, "30 fps"); label(p, cards[4].0 + 21.0, cards[4].1 + 167.0, "品質"); field(p, cards[4].0 + 168.0, cards[4].1 + 150.0, 335.0, "高品質");
    label(p, cards[5].0 + 21.0, cards[5].1 + 68.0, "プラグインの管理"); field(p, cards[5].0 + 285.0, cards[5].1 + 51.0, 240.0, "プラグインフォルダを開く"); label(p, cards[5].0 + 21.0, cards[5].1 + 108.0, "利用可能なプラグイン"); field(p, cards[5].0 + 285.0, cards[5].1 + 91.0, 240.0, "⟳  プラグインをスキャン");
