...
fn draw_topbar(ui: &mut egui::Ui, width: f32, mobile: bool) {
    ...
    if !mobile && width >= 1100.0 {
        p.line_segment(
            [egui::pos2(264.0, 14.0), egui::pos2(264.0, 45.0)],
            egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE),
        );
        p.text(
            egui::pos2(291.0, 24.0),
            egui::Align2::LEFT_CENTER,
            "映像に、まだ見ぬ世界を。",
            egui::FontId::proportional(14.0),
            colors::TEXT_PRIMARY,
...
fn draw_chapters(
...
) {
    {
        ...
        p.line_segment(
            [
                egui::pos2(rect.left() + width, rect.top()),
    ...
    icons::render_svg_at(
        ui,
        "tutorial-chapter-heading".to_string(),
        icons::SVG_LAYERS,
        egui::vec2(55.0, 55.0),
        colors::TEXT_PRIMARY,
        egui::pos2(rect.left() + 30.0, rect.top() + 20.0),
    );
    let chapter_icons = [
        icons::SVG_HOME,
        icons::SVG_FILE,
    ...
    for (i, icon) in chapter_icons.into_iter().enumerate() {
        icons::render_svg_at(
            ui,
            format!("tutorial-chapter-{i}"),
            icon,
            egui::vec2(30.0, 30.0),
            if i == 0 { ORANGE } else { colors::TEXT_PRIMARY },
            egui::pos2(
                rect.left() + 28.0,
                rect.top() + 97.0 + i as f32 * if compact { 61.0 } else { 78.0 },
            ),
        );
    }
    let p = ui.painter().clone();
    let logo_rect = egui::Rect::from_min_size(
        egui::pos2(rect.left() + 36.0, rect.top() + 20.0),
        egui::vec2(55.0, 55.0),
    );
    ...
    p.text(
        egui::pos2(rect.left() + 105.0, rect.top() + 48.0),
        egui::Align2::LEFT_CENTER,
        "Kagari",
    ...
    p.text(
        egui::pos2(rect.left() + 185.0, rect.top() + 48.0),
        egui::Align2::LEFT_CENTER,
        "VFX",
        egui::FontId::proportional(27.0),
        colors::TEXT_SECONDARY,
    ...
    let chapters = [
        ...
        ("基本操作", "画面の見方・操作方法"),
        ("レイヤー", "合成の基本"),
        ("マスクとトラック", "マスク・モーショントラッキング"),
        ("エフェクト", "VFX エフェクトの使い方"),
        ("カラーグレーディング", "色調補正・ルック開発"),
        ("書き出し", "レンダリング・書き出し設定"),
        ("チュートリアル", "すべてのチュートリアル"),
    ];
    for (i, (title, sub)) in chapters.into_iter().enumerate() {
        ...
        let item = egui::Rect::from_min_size(
            ...
            egui::vec2(width - 25.0, item_h),
        );
        let response = ui.interact(
            item,
            egui::Id::new(("tutorial-chapter", i)),
            egui::Sense::click(),
        );
        if i == 0 {
            p.rect_filled(item, 7.0, egui::Color32::from_rgb(29, 32, 36));
            p.rect_filled(
                egui::Rect::from_min_size(item.min, egui::vec2(5.0, item.height())),
                3.0,
                ORANGE,
            );
        ...
        ui.put(
            egui::Rect::from_min_max(
                egui::pos2(label_left, item.top() + 3.0),
                egui::pos2(label_right, item.top() + item_h * 0.53),
            ),
            egui::Label::new(
        ...
        ui.put(
            egui::Rect::from_min_max(
                egui::pos2(label_left, item.top() + item_h * 0.49),
                egui::pos2(label_right, item.bottom() - 2.0),
            ),
            egui::Label::new(
                egui::RichText::new(sub)
                    .size(if compact { 9.0 } else { 12.0 })
                    .color(colors::TEXT_SECONDARY),
            )
            .truncate(),
        );
    ...
    let thumb = egui::Rect::from_min_max(
        egui::pos2(rect.left() + 14.0, rect.bottom() - 133.0),
        egui::pos2(rect.left() + width - 13.0, rect.bottom()),
    );
    if let Some(id) = texture(
        ctx_for(ui),
        "assets/studio/studio_city_reference.webp",
        "tutorial-nav-thumb",
    ) {
        p.image(
            id,
    ...
    p.text(
        egui::pos2(thumb.left() + 28.0, thumb.bottom() - 40.0),
        egui::Align2::LEFT_CENTER,
        "C R E A T E    C O M P O S I T E",
...
fn draw_main(
    ...
    p.text(
        egui::pos2(rect.left() + pad, rect.top() + 75.0),
        egui::Align2::LEFT_CENTER,
        "T U T O R I A L",
    ...
    p.text(
        egui::pos2(rect.left() + pad, rect.top() + 119.0),
        egui::Align2::LEFT_CENTER,
        "はじめに",
        egui::FontId::proportional(if mobile { 32.0 } else { 42.0 }),
        colors::TEXT_PRIMARY,
    );
    p.text(
        egui::pos2(rect.left() + pad, rect.top() + 157.0),
        egui::Align2::LEFT_CENTER,
        "Kagari VFX で広がる、映像表現の可能性",
        egui::FontId::proportional(if mobile { 16.0 } else { 21.0 }),
        colors::TEXT_SECONDARY,
    );
    if !mobile {
        p.text(
            egui::pos2(rect.right() - 30.0, rect.top() + 76.0),
            egui::Align2::RIGHT_CENTER,
            "V I S U A L   E F F E C T S\nF O R   A   B R I G H T E R   T O M O R R O W",
            egui::FontId::proportional(8.0),
            colors::TEXT_SECONDARY,
        );
    }
    let hero_top = rect.top() + 190.0;
    let hero_h = if mobile {
        160.0
    } else if compact {
        170.0
    } else {
    ...
    let hero = egui::Rect::from_min_max(
        egui::pos2(rect.left() + pad, hero_top),
        egui::pos2(rect.right() - pad, hero_top + hero_h),
    );
    if let Some(id) = texture(
        ctx,
        "assets/studio/studio_city_reference.webp",
    ...
    p.circle_stroke(
        hero.center(),
        hero.height().min(hero.width()) * 0.16,
        egui::Stroke::new(2.0_f32, egui::Color32::WHITE),
    );
    p.text(
        egui::pos2(hero.left() + 22.0, hero.bottom() - 38.0),
        egui::Align2::LEFT_CENTER,
        "Kagari VFX で、\n想像を超えた映像をつくる",
        egui::FontId::proportional(14.0),
        egui::Color32::WHITE,
    ...
    p.text(
        egui::pos2(hero.right() - 18.0, hero.bottom() - 22.0),
        egui::Align2::RIGHT_CENTER,
        "03:24",
    ...
    icons::render_svg_at(
        ui,
        "tutorial-hero-play".to_string(),
        icons::SVG_PLAY,
        egui::vec2(42.0, 42.0),
        egui::Color32::WHITE,
        egui::pos2(hero.center().x - 21.0, hero.center().y - 21.0),
    );
    let timeline_top = hero.bottom() + 18.0;
    let timeline_h = if mobile {
        96.0
    } else if compact {
        90.0
    } else {
    ...
    draw_timeline(
        ui,
        egui::Rect::from_min_max(
            egui::pos2(rect.left() + pad, timeline_top),
            egui::pos2(rect.right() - pad, timeline_top + timeline_h),
        ),
        mobile,
    );
    let p = ui.painter();
    let button_w = if mobile {
        rect.width().min(320.0)
    } else if compact {
        320.0
    } else {
        388.0
    ...
    let button = egui::Rect::from_min_size(
        egui::pos2(
            rect.center().x - button_w * 0.5,
            timeline_top + timeline_h + if compact { 14.0 } else { 22.0 },
        ),
    ...
    p.rect_filled(
        ...
        10.0,
        if response.hovered() {
            egui::Color32::from_rgb(255, 125, 32)
        } else {
            ORANGE
        },
    );
    p.text(
        egui::pos2(button.left() + 82.0, button.center().y),
        egui::Align2::LEFT_CENTER,
        "チュートリアルを再生",
        egui::FontId::proportional(if mobile { 18.0 } else { 21.0 }),
        egui::Color32::WHITE,
    );
    p.text(
        egui::pos2(
            rect.center().x,
            button.bottom() + if compact { 20.0 } else { 30.0 },
        ),
        egui::Align2::CENTER_CENTER,
        "Kagari VFX の基本とワークフローを動画で学びましょう",
        egui::FontId::proportional(13.0),
        colors::TEXT_SECONDARY,
    );
    let _ = p;
    icons::render_svg_at(
        ui,
        "tutorial-button-play".to_string(),
        icons::SVG_PLAY,
        egui::vec2(22.0, 22.0),
        egui::Color32::WHITE,
        egui::pos2(button.left() + 48.0, button.center().y - 11.0),
    );
    if response.clicked() {
...
fn draw_timeline(ui: &mut egui::Ui, rect: egui::Rect, mobile: bool) {
    ...
    p.text(
        egui::pos2(rect.left() + 16.0, rect.top() + 20.0),
        egui::Align2::LEFT_CENTER,
        "➜  レイヤー",
        egui::FontId::proportional(13.0),
        colors::TEXT_PRIMARY,
    ...
    for i in 0..6 {
        p.text(
            egui::pos2(ruler_x + i as f32 * 65.0, rect.top() + 24.0),
            egui::Align2::CENTER_CENTER,
            format!("00:{:02}", i * 2),
    ...
    let lane_colors = [
        egui::Color32::from_rgb(98, 73, 186),
        egui::Color32::from_rgb(52, 153, 123),
        egui::Color32::from_rgb(107, 87, 188),
        egui::Color32::from_rgb(181, 139, 57),
        egui::Color32::from_rgb(51, 117, 195),
    ];
    let lane_step = if mobile || rect.height() < 150.0 {
        13.0
    } else {
        rect.height().min(190.0) / 6.0
    };
    for (i, label) in labels.into_iter().enumerate() {
        let y = rect.top()
            + if mobile || rect.height() < 150.0 {
                32.0
            } else {
                47.0
            }
        ...
        p.line_segment(
            [
                egui::pos2(rect.left() + 8.0, y + 15.0),
                egui::pos2(rect.right() - 8.0, y + 15.0),
            ],
            egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE),
        );
        p.text(
        ...
        let bar = egui::Rect::from_min_size(
            egui::pos2(bar_x - i as f32 * 40.0, y - 9.0),
            egui::vec2(
                (rect.width() * 0.33 + i as f32 * 22.0).min(rect.right() - bar_x),
                18.0,
            ),
        );
...
fn draw_about(ui: &mut egui::Ui, rect: egui::Rect, left: f32, _width: f32, compact: bool) {
    ...
    let panel = egui::Rect::from_min_max(
        egui::pos2(left + 20.0, rect.top() + 28.0),
    ...
    p.text(
        egui::pos2(x, panel.top() + 54.0),
        egui::Align2::LEFT_CENTER,
        "Kagari VFX とは",
        egui::FontId::proportional(if compact { 25.0 } else { 32.0 }),
        colors::TEXT_PRIMARY,
    );
    p.line_segment(
        [
            egui::pos2(x, panel.top() + 87.0),
            egui::pos2(x + 55.0, panel.top() + 87.0),
        ],
        egui::Stroke::new(2.0_f32, ORANGE),
    );
    let desc = "Kagari VFX は、映像制作のための\nレイヤーベース VFX・コンポジットソフトです。\n直感的な操作と高品質なエフェクトで、\n映画のような映像表現を、すべてのクリエイターに。";
    p.text(
        egui::pos2(x, panel.top() + 125.0),
    ...
    p.line_segment(
        [
            egui::pos2(x, panel.top() + 225.0),
            egui::pos2(panel.right() - 32.0, panel.top() + 225.0),
        ],
        egui::Stroke::new(1.0_f32, colors::BORDER_SUBTLE),
    );
    p.text(
        egui::pos2(x, panel.top() + 258.0),
        egui::Align2::LEFT_CENTER,
        "このチュートリアルで学べること",
        egui::FontId::proportional(if compact { 16.0 } else { 19.0 }),
        colors::TEXT_PRIMARY,
    );
    let items = [
        ("インターフェースの概要", "作業画面の構成と各パネルの役割"),
        ("レイヤーの基本", "レイヤーの管理と合成の仕組み"),
        (
            "マスクとトラッキング",
            "マスク作成とモーショントラッキングの使い方",
        ),
        ("エフェクトの適用", "豊富なVFXエフェクトで映像を強化"),
        (
            "カラーグレーディング",
            "色調補正とシネマティックなルックの作り方",
        ),
        ("レンダリングと書き出し", "最終的な映像の出力設定"),
        ("実践テクニック", "プロのワークフローから学ぶ応用テクニック"),
    ];
    ...
    p.text(
        egui::pos2(panel.right() - 30.0, panel.bottom() - 28.0),
        egui::Align2::RIGHT_CENTER,
        "Create\nWithout Limits.",
...
fn draw_footer(ui: &mut egui::Ui, width: f32, mobile: bool) {
    ...
    p.line_segment(
        [
            egui::pos2(width * 0.325, 34.0),
            egui::pos2(width * 0.39, 34.0),
        ],
        egui::Stroke::new(1.0_f32, colors::TEXT_SECONDARY),
    );
    p.text(
        egui::pos2(width * 0.5, 34.0),
        egui::Align2::CENTER_CENTER,
        "チュートリアル画面",
    ...
    p.line_segment(
        [
            egui::pos2(width * 0.61, 34.0),
            egui::pos2(width * 0.675, 34.0),
        ],
        egui::Stroke::new(1.0_f32, colors::TEXT_SECONDARY),
    );
    p.text(
        egui::pos2(width - 38.0, 34.0),
        egui::Align2::RIGHT_CENTER,
...
fn ctx_for(ui: &egui::Ui) -> &egui::Context {
    ui.ctx()
}
