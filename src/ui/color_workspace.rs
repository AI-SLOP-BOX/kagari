...
pub fn draw(app: &mut KagariApp, ctx: &egui::Context) {
    ...
    let sidebar_w = if narrow { 150.0 } else { 231.0 };
    let shot_w = if narrow { 190.0 } else { 219.0 };
    let right_w = if narrow { 370.0 } else { 521.0 };
...
fn draw_shots(ui: &mut egui::Ui, ctx: &egui::Context, rect: egui::Rect) {
    ...
    for (i, (num, name, time)) in rows.into_iter().enumerate() {
        let y = rect.top() + 55.0 + i as f32 * 70.0;
        let active = i == 0;
        if active {
            p.rect(
                egui::Rect::from_min_size(
                    egui::pos2(rect.left() + 10.0, y),
                    egui::vec2(rect.width() - 20.0, 68.0),
                ),
                6.0,
        ...
        if let Some(id) = asset(
            ...
            p.image(
                id,
                egui::Rect::from_min_size(
                    egui::pos2(rect.left() + 18.0, y + 9.0),
                    egui::vec2(66.0, 48.0),
                ),
        ...
        p.text(
            egui::pos2(rect.left() + 93.0, y + 17.0),
            egui::Align2::LEFT_CENTER,
            num,
        ...
        p.text(
            egui::pos2(rect.left() + 93.0, y + 37.0),
            egui::Align2::LEFT_CENTER,
            name,
        ...
        p.text(
            egui::pos2(rect.left() + 93.0, y + 54.0),
            egui::Align2::LEFT_CENTER,
            time,
...
fn draw_viewer(ui: &mut egui::Ui, ctx: &egui::Context, rect: egui::Rect, lower_top: f32) {
    ...
    let image = egui::Rect::from_min_max(
        ...
        egui::pos2(rect.right() - 14.0, rect.top() + 328.0),
    );
    ...
    p.text(
        egui::pos2(rect.left() + 18.0, rect.top() + 355.0),
        egui::Align2::LEFT_CENTER,
        "00:00:12:17",
    ...
    p.text(
        egui::pos2(rect.left() + 60.0, ty + 63.0),
        egui::Align2::CENTER_CENTER,
    ...
    p.rect_filled(
        egui::Rect::from_min_size(
            egui::pos2(rect.left() + 59.0, ty + 78.0),
            egui::vec2(rect.width() - 75.0, 42.0),
        ),
        4.0,
        egui::Color32::from_rgb(23, 39, 50),
    );
    p.text(
        egui::pos2(rect.left() + 20.0, ty + 98.0),
        egui::Align2::LEFT_CENTER,
        "V1",
        egui::FontId::proportional(13.0),
        colors::TEXT_PRIMARY,
    ...
    p.text(
        egui::pos2(rect.left() + 20.0, ty + 145.0),
        egui::Align2::LEFT_CENTER,
        "A1",
        egui::FontId::proportional(13.0),
        colors::TEXT_PRIMARY,
    ...
    p.rect_filled(
        egui::Rect::from_min_size(
            egui::pos2(rect.left() + 59.0, ty + 84.0),
            egui::vec2(rect.width() - 90.0, 30.0),
        ),
        3.0,
    ...
    p.rect_filled(
        egui::Rect::from_min_size(
            egui::pos2(rect.left() + 59.0, ty + 128.0),
            egui::vec2(rect.width() - 80.0, 25.0),
        ),
        3.0,
    ...
    p.line_segment(
        [
            egui::pos2(rect.left() + rect.width() * 0.42, ty + 70.0),
            egui::pos2(rect.left() + rect.width() * 0.42, ty + 165.0),
        ],
        egui::Stroke::new(2.0_f32, egui::Color32::from_rgb(255, 107, 22)),
    );
}
...
fn draw_lower(ui: &mut egui::Ui, _ctx: &egui::Context, rect: egui::Rect, narrow: bool) {
    let p = ui.painter();
    let w = (rect.width() - 30.0) / if narrow { 2.0 } else { 4.0 };
    let cards = [
        (rect.left() + 10.0, w, "カラーホイール"),
        (rect.left() + 20.0 + w, w, "カーブ"),
        (rect.left() + 30.0 + w * 2.0, w, "LUT"),
        (rect.left() + 40.0 + w * 3.0, w, "ショットマッチ"),
    ];
    for (i, (x, cw, title)) in cards.into_iter().enumerate() {
        let b = egui::Rect::from_min_size(
            egui::pos2(x, rect.top() + 10.0),
            egui::vec2(cw - 10.0, rect.height() - 20.0),
        );
        p.rect(
            b,
            7.0,
        ...
        if i == 0 {
            for j in 0..3 {
                let c = egui::pos2(
                    b.left() + 72.0 + j as f32 * (b.width() - 120.0) / 2.0,
                    b.top() + 126.0,
                );
                p.circle_stroke(
                    c,
                    50.0,
                    egui::Stroke::new(5.0_f32, egui::Color32::from_rgb(37, 191, 179)),
                );
                p.circle_filled(c, 5.0, egui::Color32::from_rgb(210, 220, 230));
                p.text(
                    egui::pos2(c.x, b.top() + 57.0),
                    egui::Align2::CENTER_CENTER,
                    ["リフト", "ガンマ", "ゲイン"][j],
                    egui::FontId::proportional(12.0),
                    colors::TEXT_PRIMARY,
            ...
            p.line_segment(
                [
                    egui::pos2(b.left() + 25.0, b.bottom() - 35.0),
                    egui::pos2(b.right() - 25.0, b.top() + 55.0),
                ],
                egui::Stroke::new(2.0_f32, egui::Color32::WHITE),
            );
        } else if i == 2 {
            for j in 0..3 {
                for k in 0..3 {
                    let x2 = b.left() + 45.0 + j as f32 * 70.0;
                    let y2 = b.top() + 64.0 + k as f32 * 56.0;
                    p.rect(
                        egui::Rect::from_min_size(egui::pos2(x2, y2), egui::vec2(62.0, 42.0)),
                        4.0,
