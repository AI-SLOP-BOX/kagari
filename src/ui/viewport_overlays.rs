...
pub fn draw_viewport_overlays(
    ...
    if app.show_grid {
        ...
        let grid_stroke = egui::Stroke::new(0.5_f32, colors::GRID_LINE);
        let mut gx = origin_x + grid_step;
        while gx < origin_x + draw_w {
            ui.painter().line_segment(
                [egui::pos2(gx, origin_y), egui::pos2(gx, origin_y + draw_h)],
                grid_stroke,
            );
            gx += grid_step;
        }
        ...
        while gy < origin_y + draw_h {
            ui.painter().line_segment(
                [egui::pos2(origin_x, gy), egui::pos2(origin_x + draw_w, gy)],
                grid_stroke,
            );
            gy += grid_step;
        }
    ...
    if let Some(idx) = app.selection.selected_layer_idx {
        if idx < comp.layers.len() {
            ...
            if let Some(kfs) = layer.transform.position.keyframes() {
                if kfs.len() >= 2 {
                    let path_stroke = egui::Stroke::new(1.5_f32, colors::MOTION_PATH);
                        ...
                        for s in 1..=steps {
                            ...
                            let eased_t = match &a.interpolation {
                                ...
                                crate::core::keyframe::InterpolationType::Bezier {
                                    ...
                                    crate::core::keyframe::solve_bezier_eased_time(
                                        raw_t, c[0], c[1], c[2], c[3],
                                    )
                                }
                            };
                            let val = [
                    ...
                    for seg in kfs.windows(2) {
                        let (a, b) = (&seg[0], &seg[1]);
                        if let crate::core::keyframe::InterpolationType::Bezier {
                            custom_bezier,
                            ..
                        } = &a.interpolation
                        {
                            ...
                            let handle_stroke = egui::Stroke::new(
                                1.0_f32,
                                colors::MOTION_PATH.linear_multiply(0.5),
                            );
                            ui.painter()
                                .line_segment([a_screen, out_screen], handle_stroke);
                            ui.painter().circle_filled(
                                out_screen,
                                2.5,
                                colors::MOTION_PATH.linear_multiply(0.6),
                            );
                            ...
                            ui.painter()
                                .line_segment([b_screen, in_screen], handle_stroke);
                            ui.painter().circle_filled(
                                in_screen,
                                2.5,
                                colors::MOTION_PATH.linear_multiply(0.6),
                    ...
                    ui.painter()
                        .circle_filled(cp, 3.0, colors::TIMELINE_PLAYHEAD);
                    ui.painter().circle_stroke(
                        cp,
                        3.0,
                        egui::Stroke::new(1.0_f32, egui::Color32::BLACK),
    ...
    // Puppet pin trajectory paths for the selected layer
    {
        let to_screen = |v: [f32; 2]| {
            egui::pos2(
                origin_x + (v[0] / comp_w) * draw_w,
                origin_y + (v[1] / comp_h) * draw_h,
            )
        };
        let draw_v2_path = |kfs: &[crate::core::keyframe::Keyframe<[f32; 2]>],
            ...
            for kf in kfs {
                ...
                painter.circle_stroke(p, dot_r, egui::Stroke::new(0.8_f32, egui::Color32::BLACK));
            }
        };
            ...
            if let Some(layer) = comp.layers.get(idx) {
                for pin in &layer.puppet_pins {
                    if let Some(pkfs) = pin.position.keyframes() {
                        draw_v2_path(
                            pkfs,
                            egui::Stroke::new(1.2_f32, colors::MOTION_PATH.linear_multiply(0.45)),
    ...
    if app.show_guides {
        let guide_stroke = egui::Stroke::new(1.0_f32, colors::GUIDE_LINE);
        ...
        // Center Crosshair
        ui.painter().line_segment(
            [egui::pos2(cx - 10.0, cy), egui::pos2(cx + 10.0, cy)],
            guide_stroke,
        );
        ui.painter().line_segment(
    ...
    if rendered_gpu {
        ...
        if comp_now.resolve_camera().dof_enabled
            && comp_now
                .layers
                .iter()
                .any(|l| l.is_active(current_frame) && l.is_3d)
        {
        ...
        if !reasons.is_empty() {
            ...
            ui.painter().rect_filled(
                warn_rect,
                4.0,
                egui::Color32::from_rgba_unmultiplied(30, 24, 8, 210),
            );
    ...
    if app.motion_sketch_active {
        ...
        ui.painter().rect_filled(
            ms_rect,
            4.0,
            egui::Color32::from_rgba_premultiplied(50, 10, 10, 220),
        );
        ui.painter()
            .rect_stroke(ms_rect, 4.0, egui::Stroke::new(1.0_f32, colors::ACCENT_RED));
        ui.painter().text(
            ms_rect.center(),
            egui::Align2::CENTER_CENTER,
            "● MOTION SKETCH",
    ...
    if app.show_handles {
        if let Some(s_idx) = app.selection.selected_layer_idx {
            ...
            if s_idx < comp.layers.len() {
                ...
                ui.painter()
                    .line_segment([center, x_end], egui::Stroke::new(2.5_f32, colors::GIZMO_X));
                ui.painter().text(
                    egui::pos2(x_end.x + 8.0, x_end.y),
                ...
                ui.painter()
                    .line_segment([center, y_end], egui::Stroke::new(2.5_f32, colors::GIZMO_Y));
                ui.painter().text(
                    egui::pos2(y_end.x, y_end.y + 8.0),
                ...
                ui.painter()
                    .line_segment([center, z_end], egui::Stroke::new(2.5_f32, colors::GIZMO_Z));
                ui.painter().text(
                ...
                ui.painter().rect_stroke(
                    bbox_rect,
                    0.0,
                    egui::Stroke::new(1.5_f32, colors::BBOX_STROKE),
                );
                ...
                for hp in bbox_corners {
                    ...
                    ui.painter().rect_stroke(
                        h_rect,
                        1.0,
                        egui::Stroke::new(if is_hovered { 2.0_f32 } else { 1.0_f32 }, stroke_c),
                    );
                }
                ...
                ui.painter()
                    .circle_filled(center, c_radius, colors::CENTER_DOT);
                ui.painter().circle_stroke(
                    center,
                    c_radius,
                ...
                if is_center_hovered {
                    ui.painter().circle_stroke(
                        center,
                        30.0,
                        egui::Stroke::new(1.5_f32, colors::CENTER_HOVER_RING),
                    );
                }
                ...
                if app.viewport_drag_state.is_some() || app.viewport_mask_drag_state.is_some() {
                    if let Some(ptr) = hover_pos {
                        ...
                        ui.painter().rect_stroke(
                            hud_rect,
                            4.0,
                            egui::Stroke::new(1.0_f32, colors::HUD_STROKE),
                        );
                        ui.painter().text(
                            hud_rect.center(),
                            egui::Align2::CENTER_CENTER,
                ...
                if s_layer.is_3d {
                    ...
                    ui.painter().line_segment(
                        ...
                        egui::Stroke::new(2.5_f32, colors::GIZMO_X),
                    );
                    ui.painter().text(
                    ...
                    ui.painter().line_segment(
                        ...
                        egui::Stroke::new(2.5_f32, colors::GIZMO_Y),
                    );
                    ui.painter().text(
    ...
    if crate::ui::viewport_state::is_comparing(ctx) {
        ...
        ui.painter().line_segment(
            [
                egui::pos2(wipe_x, origin_y),
                egui::pos2(wipe_x, origin_y + draw_h),
            ],
            egui::Stroke::new(2.5_f32, colors::ACCENT_CYAN),
        );
        ui.painter()
            .circle_filled(handle_center, 10.0, colors::ACCENT_CYAN);
        ui.painter().circle_stroke(
            handle_center,
            10.0,
        ...
        let handle_rect =
            egui::Rect::from_center_size(handle_center, egui::vec2(24.0, draw_h.max(24.0)));
        let wipe_response = ui.interact(
            handle_rect,
            ui.id().with("viewport_wipe_drag"),
            egui::Sense::drag(),
        );
