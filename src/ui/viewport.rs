...
fn software_preview_required(comp: &crate::core::timeline::Composition, frame: u32) -> bool {
    ...
    comp.layers
        ...
        .any(|layer| {
            ...
            !layer.puppet_pins.is_empty()
                || layer.masks.iter().any(|mask| {
                    mask.enabled
                        && mask.mode != crate::core::mask::MaskMode::None
                        && (mask.expansion.evaluate(frame).abs() > 0.01
                            || mask.wiggle.is_some())
                }) || matches!(
                layer.layer_type,
...
pub fn draw(app: &mut KagariApp, ctx: &egui::Context, current_frame: u32) {
    ...
    egui::CentralPanel::default().show(ctx, |ui| {
        ...
        let frame_blending_required = app
            .history
            .current()
            .active_composition()
            .layers
            .iter()
            .any(|layer| {
                layer.frame_blending
                    && matches!(layer.layer_type, crate::core::timeline::LayerType::Video { .. })
            });
        ui.horizontal(|ui| {
...
mod review_regression_tests {
    ...
    #[test]
    fn puppet_deformation_uses_software_preview_until_gpu_warp_is_supported() {
        let mut comp = crate::core::timeline::Composition::new(
            "comp".to_string(),
            "Comp".to_string(),
            320,
