...
| Preview | Cached frames, RAM preview, adaptive quality, audio sync | 🟡 | Cache and playback exist; GPU preview explicitly falls back for unsupported blend modes, Shape Freeform/Stroke/3D geometry, layer styles, multi-effect stacks, and effect variants whose GPU parameter mapping diverges from export (including motion blur, film grain, fractal noise, Tritone, Glow/GlowPro, Hue/Saturation, and several distortions); true real-time GPU effect processing is still limited |
