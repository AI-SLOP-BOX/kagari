...
| Roto / paint | Roto Brush, paint, clone, eraser, puppet | 🟡 | Roto Brush source strokes persist and remain isolated by layer identity; segmentation samples the selected layer alone on transparent background, ignoring its existing mask and track-matte inputs so other layers cannot contaminate the foreground/background model. Tracker propagation uses the selected point; a separate cancellable optical-flow path now warps boundary vertices through every adjacent source frame in both directions, rejects inconsistent forward/backward vectors, and writes undoable per-frame keys without blocking the UI. The flow path deforms the contour but does not re-segment occluded image content; matte review/cache, color decontamination, and paint/clone/puppet parity remain well below AE |
