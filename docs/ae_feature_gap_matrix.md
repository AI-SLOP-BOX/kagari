...
| Roto / paint | Roto Brush, paint, clone, eraser, puppet | 🟡 | Roto Brush source strokes persist and remain isolated by layer identity; segmentation samples the selected layer alone on transparent background, ignoring its existing mask and track-matte inputs so other layers cannot contaminate the foreground/background model. Foreground/background strokes are now frame-scoped (legacy strokes without frame metadata remain global), and corrected contours are stored as undoable Hold keyframes so differing polygon vertex counts never interpolate into invalid geometry. Tracker propagation uses the selected point; a separate cancellable optical-flow path warps boundary vertices through every adjacent source frame in both directions, rejects inconsistent forward/backward vectors, and writes undoable per-frame keys without blocking the UI. The flow path deforms the contour but does not re-segment occluded image content; matte review/cache, color decontamination, and paint/clone/puppet parity remain well below AE |
| Content-Aware Fill | Remove an object across a sequence with generated replacement frames | 🟡 | Object (nearest-boundary propagation), Surface (boundary-average patch), and Edge Blend (masked-region smoothing) now produce distinct results. UI generation isolates the selected layer, ignores only the removal mask while sampling, writes a persistent WebP fill patch under the project-adjacent `Assets/KagariGenerated` folder, and inserts it only for the current frame; Work Area/Entire Duration controls are not yet connected to temporal rendering, and correction, occlusion-heavy footage, temporal consistency, and quality/performance acceptance remain unproven |
...
| Preview | Cached frames, RAM preview, adaptive quality, audio sync | 🟡 | Cache and playback exist; GPU preview falls back for unsupported blend modes, Shape Freeform/Stroke/3D geometry, layer styles, multi-effect stacks, every effect whose shader math/parameter mapping is not yet verified against export, and Lens Flare linked to a scene light (the shader currently receives authored UV rather than the CPU-resolved light position); the audited direct-GPU allowlist currently includes Color Tint, Levels, unlinked Lens Flare, and RGB-only Invert, so preview correctness takes precedence over GPU acceleration while exact parity remains unfinished |
